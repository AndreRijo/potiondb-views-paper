...
To this end,  we study the behavior of the underlying NuCRDTs used for supporting different types of 
...
perform well. When considering different update ratios, for small tops, the throughput decreases with an increasing 
ratio of updates, as processing updates is more complex than reads. For larger tops, the throughput with
a larger ratio of updates increases as the overhead of returning the larger tops to the clients becomes the dominating 
...
Figure \ref{fig:crdts} shows the throughput of NuCRDTs with different numbers of replicas per NuCRDT 
(for top K,  we used K = 10). 
Results are presented in Figure~\ref{fig:crdts:perf}. The first observation is that, 
...
These views can be used to efficiently answer repetitive complex queries over data that is partitioned
over different locations, without a single location holding all data.
To support these views, we build on non-uniform CRDTs, extending previous proposals to make them practical.  
