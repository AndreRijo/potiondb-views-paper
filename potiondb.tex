...
After the loading phase completes, the clients connect to the servers and start executing queries and, optionally, updates, with the query/update ratios depending on the experiment.
...
\begin{figure}
	\centering
	\includegraphics[width=0.64\linewidth]{singleQuery/all_queries_tc}
	\vspace*{-0.75em}
...
\begin{figure}
	\centering
	\includegraphics[width=0.74\linewidth]{singleQuery/single_TC_latencies}
	\vspace*{-0.6em}
...
PotionDB's query performance is unaffected by the latency added between servers, as every server's views have the necessary information to reply to all queries.
...
Conversely, Local PotionDB is highly affected by latency between servers.
...
Moreover, global queries' high latency limits each client's throughput to about 7 queries/s - thus several thousands of clients are required to saturate Local PotionDB.
...
This is expected as queries are entirely served by views which are all in one server (view server), which gets all the query load.
...
Figure \ref{fig:single_tc_latencies} gives a more accurate representation of the latencies experienced by the clients. 
...
Since Q5 only requires data of a region, Local PotionDB performs similarly to PotionDB, as the server has the required view data to answer the query directly.
For local queries, clients only request information about their own region.
...
This would be the equivalent of, for example, in an e-commerce, querying the list of most popular products in the customer's country or region.
This is the ideal scenario for Local PotionDB, yet its performance is similar to PotionDB's - both achieve a max throughput of about 1.6M ops/s and latencies starting at 0.15ms.
...
A single query takes on average 210ms to complete as for every server, the server furthest away has a latency of 203$\sim$217ms.
...
In this scenario, when a client wants to execute an operation, it picks randomly whenever to execute a query-transaction (single query) or an update-transaction.
An update transactions consists in the creation of a new order, of its ordered items and all associated view updates.
...
A 25\%/75\% update ratio means that 25\% of all executed operations (including view updates) are updates, not that 25\% of the transactions are write-transactions.
...
Update transactions apply the commit protocol locally (Section \ref{subsec:commit}), thus requiring coordination of multiple shards (due to multiple updates per transaction), which leads to an higher latency than with queries.
...
For instance, at PotionDB's max throughput for 25\% updates, its query-only latency and the corresponding for 0\% are, respectively, 0.273ms and 0.246ms. %If space is needed, replace "For instance" with "E.g."
...
Furthermore, update transactions do not need to contact every server, as half of the orders are totally local to a region \footnote{This does not occur on the original TPC-H dataset, whose orders' items are random. This modification adds locality to orders, which is more realistic for an e-commerce scenario and helps Local PotionDB more than PotionDB.
...
Note that grouping queries too would increase throughput even more considerably.
...
Single PotionDB (Figure \ref{fig:update_rates_single_tc}) has throughput losses comparable to PotionDB's, except for 25\% updates where the percentage of loss is smaller than PotionDB's due to not having to replicate view updates.
...
However, even for 25\% updates, Single PotionDB's throughput is less than 1/3 of PotionDB's.
...
%In summary, PotionDB still scales well with modest update rates as is common in scenarios like e-commerce applications,
...
All the scenarios observed so far had latency imposed between servers to simulate a geo-distributed scenario.
...
With Local PotionDB, when considering only local queries like Q5 in Figure \ref{fig:q3_q5_noTC}, the performance is the same as previously observed, given those requests are served locally.
...
\begin{figure}
	\centering
	\includegraphics[width=0.65\linewidth]{singleQuery/upd_rate_noTC_global_vs_local}
	\vspace*{-0.65em}
...
PotionDB is the same as previously observed in Figure \ref{fig:update_rates_global}. %, given latency between servers does not affect PotionDB's performance.
...
\begin{enumerate*}[label=(\roman*)]
	...
	\item orders' locality, thus many updates are fully local;
	...
	\item view updates do not need to be replicated as views are local, thus slightly reducing server load.
...
Finally, Single PotionDB's thoughput is similar to what was observed in Figure \ref{fig:update_rates_single_tc}, but with lower latencies ranging from 0.24ms to 0.6ms.
...
As a side note, PotionDB scales further if clients issue multiple queries per transaction - e.g., with 5 queries per transaction, PotionDB's max throughput (resp. transaction latency) goes from 1.5M ops/s (0.37 ms) to 4.83M ops/s (0.68 ms), an increase of 222\% (83.8\%).
...
In this scenario we drop the TPC-H dataset and use instead micro-benchmarks.
...
Unless otherwise mentioned, each client executes 5 operations per transaction, in order to avoid communication overheads of small messages dominating our experiments.
...
Top1, Top10 and Top100 all perform equally with 1 read per txn, as communication overhead for small messages (latency, metadata, serialization and deserialization of protobufs) and high number of clients dominate over costs of data transfer and query execution time.
...
E.g., Top 10 with 0\% to 25\% updates goes from 8.19M to 2.23M (-72.8\%) and 1.7M to 1.12M (-34.1\%) ops/s for, respectively, 5 and 1 operations per transaction.
