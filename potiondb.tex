...
\begin{algorithm}
	\begin{algorithmic}[1]
		\Statex vc$_i$: VC \Comment{latest applied clock of sh$_j$}
		\Statex prep$_i$: TS \Comment{list of (sorted) prepared timestamps\carla{or transactions}}
		\Statex hold$_i$ \Comment{list of (sorted) commits on hold} \carla{list of what? transactions?}%commits that have been confirmed but not yet applied (due to some timestamp in prepi that is smaller or another timestamp in holdi with a smaller value)
		\Statex txUpds: tID $\to$ \code{list}$\langle$Upds$\rangle$ \Comment{Transaction updates}
		...
		\State ts  $\leftarrow$ (pc, j) \carla{what is pc?} \Comment{}
		...
		\carla{assignment in line 6, makes the condition false}
		...
		\Statex \hspace*{1.5em} $\cdots$ \Comment{apply the updates to the CRDTs}
		...
		\Comment{received by \Call{\textls[-30]{globalClockRoutine}}{}}
		\EndFunction
		...
		\State \textbf{return} 	$\mathit{objs[objId]}$.\Call{read}{txUpds[txId]}	\label{alg_line:read}
		\Else
