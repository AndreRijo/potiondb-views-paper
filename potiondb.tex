...
\begin{algorithm}
	\begin{algorithmic}[1]
		\Statex svID,  shID  \Comment{server and shard identifiers}
		\Statex \textbf{input}
		\Statex $\quad$ globalVC: vClock  \Comment{global vector clock of $S_j$}
		...
		\Function{startTransaction}{clientVC: vClock} \label{alg_line:startTransaction}
		...
		\EndFunction
		\Function{updateOp}{txId: ID, objId: (key, bucket, type), op: Update}
		\State txInfo[txId] $\leftarrow$ txnInfo[txId] $\cup$ op$^{\mathit{shard}}$	
		...
		\State \textbf{send} \Call{update}{txId, objId, op} \textbf{to} op$^{\mathit{shard}}$
		\EndFunction
		\Function{readOp}{txId: ID, readVC: vClock, objId: (key, bucket, type), op: Read} \label{alg_line:readTM}
		\State \textbf{send} \Call{read}{txId, readVC, objId, op} \textbf{to} op$^{\mathit{shard}}$
		\EndFunction
		\Function{commitTx}{txId: ID, readVC: vClock}
		...
		\EndFor
		\State vcG[j]  $\leftarrow$ \Call{max}{vcG[j], T.cc}
...
\begin{algorithm}
	\begin{algorithmic}[1]
		\State $\mathit{vc_i}$ \Comment{latest applied clock of $\mathit{sh_j}$}
		...
		\Function{prepare}{Id $\mathit{txId}$} \label{alg_line:prepstart}
		\State $\mathit{ts} = (\mathit{pc}, i)$
		\State $\mathit{prep_i}$.\Call{sortedAdd}{$\mathit{ts, txId}$}
		\State reply to $\mathit{TM}$ with $\mathit{ts}$	\label{alg_line:prepfinish}
		\EndFunction
		\Function{commit}{$\mathit{\textrm{Id } txId, \textrm{Clock } rc, \textrm{Timestamp } cc}$}
		\State $\mathit{minPrep = prep_i}$.\Call{min}{}().$\mathit{txId}$
		\If{\Call{canCommit}{$\mathit{txId, cc}$}}	\label{alg_line:call_canCommit}
		\State \Call{applyCommit}{$\mathit{txId, rc, cc}$}
		\Else
		\State $\mathit{hold_i}$.\Call{sortedAdd}{$\mathit{cc, txId, rc}$}
		\EndIf
		\If{$\mathit{minPrep \neq prep_i.}$\Call{min}{}().$\mathit{txId}$}	
		\LeftComment{If $\mathit{txId}$ was the lowest prepare, then possibly the lowest commit in $\mathit{hold_i}$ may be appliable now.}
		\State \Call{checkHoldCommits}{}() \label{alg_line:call_checkHold}	%Even if txID’s was put on hold, it may have been the lowest prepare and, thus, another commit on hold may be OK to be applied now
		\EndIf 
		\EndFunction
		\Function{canCommit}{$\mathit{\textrm{Id } txId, \textrm{Timestamp } maxTs}$} \label{alg_line:start_canCommit}
		\State $\mathit{prep_i}$.\Call{remove}{$\mathit{txId}$}	\label{alg_line:removePrep}
		...
		\Function{checkHoldCommits}{}()	\label{alg_line:start_checkHold}
		...
		\State $\mathit{vc_i[j] = maxTs}$
		\State $\mathit{txUpds[txId] = \bot}$
		\State reply to $\mathit{TM}$ (for \Call{globalClockRoutine}{}) \label{alg_line:applyCommitReply}
		\EndFunction
		\Function{update}{$\mathit{\textrm{Id } txId, \textrm{(key, bucket, type) } objId, \textrm{Update } op}$}
		\State $\mathit{txUpds[txId]}$.\Call{append}{$\mathit{op}$}
		\EndFunction
		\Function{applyRead}{$\mathit{\textrm{Id } txId, \textrm{Clock } rc, \textrm{(key, bucket, type) } objId}$, Read  $\mathit{op}$}	\label{alg_line:readShard}
		\If{$\mathit{rc \geq objs[objId]}$.\Call{lastVersion}{}()}
		\State \textbf{return} 	$\mathit{objs[objId]}$.\Call{read}{$\mathit{txUpds[txId]}$}	\label{alg_line:read}
		\EndIf
		\State \textbf{return} $\mathit{objs[objId]}$.\Call{readInThePast}{$\mathit{txUpds[txId]}$}	\label{alg_line:readInThePast}
		\EndFunction
	\end{algorithmic}
