...
We identify two types of values: base objects, $Objs$, and derived objects, $Views$.
We define the set of objects of a database, $DB$, as $DB$ = $Objs \cup Views$.
...
The value of a view is defined as a function of the values of other object(s), i.e., $\forall v \in Views, v = fun_v(D_v)$, where \emph($D_v$) is the set of objects (potencially both base objects and other views) used to compute $v$.
We assume the computation is non-recursive, i.e., the value of a view is never based on its own value or, formally, $\forall v \in Views, v \notin D_v$.

All objects are uniquely identified by the triple (key, bucket, type).
For presentation purposes, we assume the triple is a property of the object and we define $id = (key, bucket, type)$.
Whenever needed, we represent the aforementioned properties for a given object $o$ as $o^{id}$, $o^{key}$, $o^{bucket}$, $o^{type}$.
...
\begin{table}[]
	\begin{tabular}{ll} 
		\toprule
		get(txId, id) $\rightarrow$ value                  & begin(clk) $\rightarrow$ txId            \\
		read(txId, id, op) $\rightarrow$ value             & commit(txId) $\rightarrow$ clk           \\
		update(txId, id, op) $\rightarrow$ ok           & rollback(txId) $\rightarrow$ ok                           \\
		& \\
		readTx(clk, reads...) $\rightarrow$ clk, values... & updateTx(clk, updates) $\rightarrow$ clk \\
		\bottomrule
	\end{tabular}
	\vspace{1em}
...
PotionDB provides the following API for its applications, also summarized in Table \ref{table:PotionDB_API}:

\begin{compactitem}
	\item begin(clk) $\rightarrow$ txId: starts a transaction and returns an identifier ($txId$) to represent it. The $txId$ is then used to associate reads and updates to this transaction. $Clk$ is used to ensure applications see a casually evolving state of the database. We detail this further in Section \ref{sec:transactions};
	\item get(txId, id) $\rightarrow$ value: returns the full state of object $o^{id}$ in the context of transaction $txId$. E.g., if $o^{id}$ is a set, then all its elements are returned;
	\item read(txId, id, op) $\rightarrow$ value: executes a read operation $op$ on object $o^{id}$ in the context of transaction $txId$, returning the result of the operation. E.g., if $o^{id}$ is a set, we could execute a $lookup(a)$ operation to check if the element $a$ is part of the set;
	\item update(txId, id, op) $\rightarrow$ ok: associates an update of object $o^{id}$ to the transaction $txId$. $Op$ represents the operation to be applied to modify the object's state. Updates are only executed when a commit is issued. If $o^{id}$ does not exist, it is created during the commit;
	\item commit(txId) $\rightarrow$ clk: commits the transaction $txId$, applying all updates associated to it. The clock generated by commit is returned for use in later transactions;
	\item abort(txId) $\rightarrow$ ok: cancels the transaction $txId$, discarding all updates associated to the transaction;
	\item readTx(clk, reads...) $\rightarrow$ clk, values...: executes a read-only transaction with one or more reads;
	\item updateTx(clk, updates...) $\rightarrow$ clk: executes a write-only transaction with one or more updates.
\end{compactitem}

We provide ``static'' read-only (readTx) and write-only (updateTx) transactions, as well as dynamic transactions (begin, commit, abort) where both reads and writes can be mixed.
...
Without dynamic transactions, we could first read user B as offline, then user B comes online and only afterwards the publication is done.
...
CRDTs (Conflict-free Replicated Data Types) are designed to be used in distributed systems, ensuring the states of the replicas converge despite the occourence of concurrent updates.
Many CRDTs have been proposed, allowing for a rich library of objects such as counters, registers, sets, maps, topK (leaderboard), etc.
CRDTs support operations to directly manipulate the object's state, e.g., increments on counters, instead of direct assigments.
Thus, application developers can pick the CRDTs corresponding to the data types appropriate for their application(s), without having to worry how to handle concurrency conflicts.
Furthermore, many flavours of each data type are available which implement different policies to solve conflicts, allowing CRDTs to adapt further to each application's needs.
CRDTs, together with our transactional causal consistency support and provisioning of snapshots, ease the life of application developers, as they will encounter less anomalies compared to other weak consistency solutions.
This avoids the need for ad-hoc mechanisms that are error-prone to compensate anomalies \cite{???}.

In PotionDB we support the following datatypes: counters, registers, sets, maps, topK, average and max/min.
Our CRDTs support multiple operations and reads which ease their usage. 
E.g., in a map, both single and group additions/removals can be executed, and one can read the whole map, check if a certain key exists or get the value of a key, etc.
We further extended some CRDTs designs to make them more expressive.
E.g., our topK supports both additions and removals, as well as extra fields necessary to answer queries; we also provide a variant of topK (topSum) which supports incrementing or decrementing an existing entry.
Our maps are composable, i.e., the value of a map can be any other CRDT, which further enhances the expressivety of PotionDB.
%Does the composability need some example to justify it?

These extensions and multi-operations, as well as the variety of datatypes, makes CRDT implementation in PotionDB cumbersome, as it is necessary to ensure the states converge despite the execution of operations concurrently, as well as respect the sequential semantics of the object.
...
For instance, we implemented the TPC-H's \cite{tpch} dataset, update operations and six of their queries, including defining efficient, maintaneable views for each query.
We also analysed the remaining queries and sketched solutions for implementing all remaining queries using PotionDB's objects and views, which demonstrates PotionDB's expressivety. %Maybe I should say datatypes instead of "objects and views"?
We leave details on our TPC-H implementation for Section \ref{sec:}.
...
Defining materialized views by directly composing PotionDB's datatypes, while possible, can at times be difficult and cumbersome.
This is further aggravated by the need to keep views updated, which would imply the application developer having to write code for both updating the base objects and views.
To avoid this, we specify a language inspired in SQL \cite{sequel} to specify materialized views.
From this specification, PotionDB can automatically infeer the required objects to support the view, as well as automatically generate the necessary view updates' whenever a relevant base object is inserted, updated or deleted.
We detail this inferring in Section \ref{subsec:generated_view}.
...
\begin{figure}
	...
	\begin{minipage}{0.19\textwidth}
		...
		\end{tabular}
		\vspace{1em}
		\captionof{table}{TopSales materialized view. This view builds upon Sales objects.}
		\label{table:topSales}
	\end{minipage} \hfill
	\begin{minipage}{0.26\textwidth}
		CREATE view ($key, bucket$) \\
		SELECT attribute1, ..., attributeN \\
		FROM container \\
		WHERE condition(s) \\
		(optionally: ORDER BY, LIMIT) \\
...
\begin{figure}
	CREATE view (\emph{TopSales, views}) AS \\
	SELECT ProdName sum(Price - Cost) AS SumProfit\\
	\hphantom{SELECT }count(sales.ID) AS NumSales \\
	FROM sales \\
	WHERE SaleDate.Month = 2 and SaleDate.Year = 2023\\
...
\emph{Create} defines the key and bucket of the view.
...
\emph{Where} allows to define conditions that are used to decide whenever a given object is considered for the view or not, depending on certain properties of the object (e.g., only include European customers in the view).
\emph{Order by} sorts the rows of the view given one or more properties of the view.
\emph{Limit} indicates that this view represents a top, i.e., a leaderboard, and optionally defines a max number of elements to be considered as being in the top.

Any parameter of the intended query should be indicated between []. %Should I give an example of what this is? By parameter I mean, e.g., queries that have many variants - e.g., asking the same query for different days of the week. The day of the week is a parameter of the query.
For conditions in \emph{where}, $>$, $<$, $=$, $!=$ are supported, as well as some operations with strings (e.g., startsWith). %Should we mention in theory we could support directly comparing dates?
A simple "if then else" is also supported inside aggregates, to further boost their expressivety.	%This is useful e.g. for Q14, that way we can use one single average object instead of two.
...
The \emph{From} clause then refers to one (and only one) of those containers. %Should I mention our restriction of only one container in some other way?
...
Figure \ref{fig:topSalesSQL} shows a specification of a view which answers the following query: "Who were the top 10 most profitable customers on February? And how many products did those buy?"
Example data of the base objects and of the view can be seen, respectively, in Tables \ref{table:sales} and \ref{table:topSales}.
...
\emph{From} specifies that all objects belonging to the container \emph{sales} should be considered.
...
\emph{Where} allows for only sales done on the month of February to be considered.
Meanwhile, \emph{Order By} defines how to sort the rows, using NumSales as a tie-breaker, while \emph{Limit} specifies how many elements are considered to be "in the top".
More complex queries are also possible - namely, our specification language allows specifying efficient views for most of TPC-H's queries. %TODO: Maybe be more precise? Out of 23 queries, I have confirmed we can support 20 with our language - only Q13, Q16 and Q21 are not supported. And those 3 can still be supported in PotionDB by defining the views manually

%Should I mention what do we support in the where clause? E.g., where supports AND, OR, >=, <=, ==. We also support comparing with some variable (e.g., YEAR >= 1993 and YEAR <= 1997 and shipdate.year = YEAR). May be a few more things
...
\emph{Extensions.} We extend the specification language as follows.
On the \emph{From} clause, multiple containers are supported, as long as each object in the first container of the clause matches with, at most, one object from each of the other containers.
This allows for views to fetch data that may be in other objects that is relevant for the view.
...
Each time the respective event regarding objects of said container occours, the trigger's code is executed.
...
We also support the weaker Read Commited (RC) consistency. %Do I need to justify why we provide both? Basically the later is provided to achieve higher throughput.
...
\emph{Causality.} 
We leverage on the happens-before relationship defined by Lamport \cite{???} to define causality.
We represent than $op_1$ happened before $op_2$ as $op_1 \prec op_2$.
...
By contrast, if $op_1 \nprec op_2$ and $op_1 \nprec op_2$, then the operations are concurrent.
...
If both operations are a write for the same object, this could lead to replicas diverging forever \cite{cops, cure} (i.e., returning different results for the same read forever).
...
In our solution, we leverage on CRDTs to solve these conflits \cite{crdt}.
...
Let $S_{init}$ be the initial, empty state of the database.
We can define a snapshot $S_n$ as the state of the database after executing a sequence of operations $op_1, ..., op_n$ according to some valid serialization that respects the happens-before relationship, i.e., $S_n = op_1(...(op_n(S_{init})))$.
...
Our transactions are atomic and generate a new consistent snapshot when commited.
I.e., given any transaction $T$ executing on snapshot $S_n$ with operations $op_1, ..., op_n$, either $T$ commits and $S_t$ = $op_n(...(op_1(S_n)))$ (all operations on the new snapshot) or it aborts and $\forall (op_1, ..., op_n) \in T : op_1 \notin Ops(S_t), ..., op_n \notin Ops(S_t)$ (none of the operations are in the new snapshot), where $Ops(S_t)$ denotates the list of operations that generated the snapshot $S_t$.
 
Our PotionDB prototype provides Transactional Causal Consistency (TCC), i.e., our transactions always observe a consistent snapshot of the database.
...
This model is weaker, i.e., less guarantees, than transactional causal consistency.
However, on write-heavy scenarios (e.g., 75\%-25\% write-read), the provision of consistent snapshots for TCC is more expensive.
This is usually common for systems providing some sort of causal consistency.
E.g., Cure, Eiger, Gentlerain, COPS \cite{cure, eiger, gentlerain, cops} also have considerable performance drops when update ratios are high.
Thus, we provide RC for write-heavy scenarios where maintaining a high read throughput is a must.

Intuitively, under RC, PotionDB returns for each object whichever last version (commited) is available.
...
This scenario is common enough as we expect that most complex queries in PotionDB to be served by a single view and, thus, single read.
Besides, the application developer can always fall back to TCC when needed.

We now define precisely our guarantees under RC.
First, consecutive reads of the same object read the same state or more recent, even if under the same transaction.
I.e., consider a transaction T, reads $R_1(o) \in T$ and $R_2(o) \in T$, as well as the statues returned by them $s_1(o)$ and $s_2(o)$.
Under RC, we have that $R_1(o) \prec R_2(o) \implies s_1(o) \prec s_2(o)$
Thus causality on a single object is respected, but isolation from other transactions is not (under TCC, both reads would return the same state).
Causality across objects is also not respected.
Assume $R_1(o_1) \in T$ and $R_2(o_2) \in T$, with $o_1^{id} \neq o_2^{id}$.
Under RC, the following is possible: $R_1(o_1) \prec R_2(o_2) \land s_2 \prec s_1$.
I.e., reads of different objects may return states corresponding to different snapshots.

\section{PotionDB Overview}

[TODO]. I think this likely needs to be merged with Data model/interface. Will leave this for later.

\subsection{Architecture}
\label{subsec:architecture}

%Maybe I don't need to always refer "more details in Section x, y, z if I provide an overview in the Introduction?"
%Note: Here we refer to objects before defining objects!
...
A client's transactions are executed locally in the PotionDB's instance with which the client communicates.
Updates are propragated asynchronously to other replicas.
If a client's transaction accesses data not localy replicated, other instances with said data are contacted and involved in the transaction.
We note this should be an exceptional case, not the norm.
We discuss this further in Section \ref{subsec:operationsNonLocal}.

The internal architecture of PotionDB is inspired by Cure and can be split in three main components:

%Not sure if I'll need these abbreviations later. Consider removing if unused.
\begin{compactitem}
	\item TransactionManager (TM). This component is reponsible for receiving and replying to client's requests.
	It coordinates transactions, implementing a transactional protocol (described in Section \ref{sec:transactions}) which ensures the consistency of reads and updates, as well as the transaction's atomicity.
	\item Materializer: This component stores the objects on their latest version, as well as the necessary data to generate previous versions when needed.
	...
	The replication component is aware of each instances's replication scheme, ensuring each instance only receives, for each transaction, updates for objects it replicates. We detail this in Section \ref{subsec:partial}.
\end{compactitem}

\subsection{Sharding}
\label{subsec:sharding}

%Maybe I can mention doing partitioning based on some other criteria (e.g., to make better usage of views + objects) is left as future work/orthogonal line of research.
%I need the object definition before this section.

%Note: Redis is also single-threaded - I can use that as an example if needed too

Internally, the Materializer component is partitioned (sharded).
This allows to efficiently utilize multicore CPUs, allowing reads and updates to execute concurrently whenever possible.
...
%We also define the set of shards accessed by a Transaction $T$ as $part(T) = {o^{part} | o \in T}.
...
However, instead of running multiple instances of PotionDB and sharding the data across those instances, we shard the dataset across threads of the same database instance, as this allows more efficient communication between shards when needed.
...
When considering multiple transactions, if there's only reads then there is no conflict either, as states do not change.
However, when considering writes, if there is an overlap of updates of different transactions for the same shard, there is a conflict, as the results may be different depending on the execution order.
%I am not sure if the "intuition" above is necessary at all.

We now define some notations in order to precisely describe the scenarios where operations, i.e. reads and writes, can be executed concurrently.
First, for a given transaction $T$, let $W(T)$ and $R(T)$ be, respectivelly, the set of writes/reads present in $T$.
We say that $o \in W(T)$ (resp. $o \in R(T)$) if there exists a write (resp. read) operation in $W(T)$ (resp. $R(T)$) for object $o$.
Furthermore, $shard(W(T))$ (resp. $shard(R(T))$) returns the set of shards for which at least one object $o$ resides in each shard, i.e., $\forall s \in shard(W(T)) | \exists o^{shard} \in W(T) : shard = s$ (and similarly for $R(T)$).

\begin{compactitem}
	\item Operations for a given transaction that belong to different shards may always execute concurrently;
	\item For a given set of transactions T1, ..., Tn, if all transactions are read-only, they can execute freely in any order, exploiting full concurrency between shards;
	\item For a given set of transactions T1, ..., Tn, with a subset of the operations being writes, the transactions can execute concurrently as long as $\forall Ta, Tb \in T1, ..., Tn, Ta \neq Tb \implies (shard(W(Ta)) \cap shard(W(Tb)) = \emptyset)$ (informally, there is no intersection of shards between any of the write sets). Note that the read sets can interset freely.
\end{compactitem}
...
Alternative \ref{item:locks} is difficult to implement, as it is required to find the right level of lock granularity and when should locks be obtained/released. 
...
Many existing solutions shard the data across multiple machines in the same datacenter \cite{cops, eiger, cure, mdcc, gentlerain}.
While this implies coordination between machines for executing cross-shard transactions (e.g. with 2PC), it allows to scale the dataset size further than what a single machine can handle.
...
%I need to check if I need to introduce snapshots or clocks more properly in one of the previous sections, or here (I should check this when reviewing the entire document).
...
Each server keeps a global vector clock $vc_g$, with one entry for each server in the system.
This clock represents the latest snapshot available in the server.
Each partition $i$ also keeps a local vector clock $vc_i$.
The local vector clock simbolizes the latest snapshot available in the partition, which may be different from $vc_G$.
Any partition can access the server's physical clock, $pc$.
Each partition also keeps a list of prepared vector clocks, $prep_i$, as well as a list of commits on hold, $hold_i$
...
In fact, any monotonic increasing clock would suffice.
...
Applying a local commit only advances the replica's entry in a vector clock, hence why a single scalar is enough for $T.cc$.
...
%I may need to define some more notation or information
...
First, a prepare message is sent to all partitions in $p_w(T)$.
Each partition $i$ replies with the value of the physical clock $pc$ at the time the message is processed and also stores it in $prep_i$. %Do I need to say it also stores the transaction ID?
We assume that $pc$ is precise enough that the same $pc$ value won't be returned for two different prepares in the same partition.
It is however acceptable for two different partitions to read and return the same $pc$ even if for different transactions.
We set $T.cc$ be the highest of the returned clocks.
A commit message is then sent to all partitions in $p_T$.	%I may need to prove how this still converges - I remember me and Carla talked about this and it is not obvious.
%Also, should I just say we require a monotonic increasing clock? What we require is a bit weaker actually (just needs to be monotonic increasing on the level of a partition)

A partition $i$ proceeds as follows when it receives a commit message.
A commit can only be applied if all prepared clocks in $prep_i$, except the one prepared for $T$, are higher than $T.cc$.
If so, the commit and $T$'s updates are applied and the partition's $vc_i[server]$ is updated to $T.cc$.
Otherwise, the commit is queued in $hold_i$ to be applied later, as other transactions may commit with a lower $cc$ than $T.cc$.
In either case, $T$'s prepared clock is removed from $prep_i$ and the partition replies with a commited message.

If $T$'s commit was applied in partition $i$, or $T$'s prepare clock was the lowest value in $prep_i$, the transaction with lowest $cc$ in $hold_i$, $T_h$, is verified for appliance, with the same rules as above.
...
This process loops until $hold_i$ is empty or the lowest commit in $hold_i$ cannot be applied. %Do I need to state that vc is updated?

A client receives a confirmation of transaction commited (along with $T.cc$ which will then be piggybacked on the next transaction) when all partitions in $p_w(T)$ send the commited message.
This happens even if any of the partitions queued the commit.
On the other hand, $vc_G$ is updated only when all partitions in $p_w(T)$ actually finish executing the transaction.
To be precise, assuming the transaction was commited on server $S_j$, we set $vc_G[j] = max(vc_G[j], T.cc)$.
The max is necessary as transactions accessing disjoins sets of write partitions may commit concurrently.
...
Thus, all reads in a transaction observe a consistent view of the database, even when accessing multiple partitions.
...
This ensures causality is maintaned while reading even if the client changes servers.
...
If at the time of reading, $\exists i \in S_1, ..., S_n : T.rc[i] > vc_G[i]$, the reads will be put on hold before being sent to the partitions. 
When $\forall i \in S_1, ..., S_n, vc_G[i] >= T.rc[i]$, i.e., the replica can already provide the snapshot requested by the client's clock, the reads will be sent to the respectives partitions.
...
The partitions receive $T.rc$ when applying reads. If $T.rc$ belongs to the past, the correspondent version of the object is calculated. %Do I need to give details on this?
If it is the latest, the state is read directly.

The waiting situation can happen when the client changes servers or if the client recently commited some writes and the server is lagging behind, as PotionDB replies to the client as soon as it is known that a transaction will be comitted.
...
%That is, even though updates are not applied until a transaction is commited, reads still reflect the state that would result from applying said updates on $T.rc$'s snapshot.
...
In our prototype we achieve this by, for a given $read(o)$, calculating the effects of all $update(o) \in T$ that have been issued before said read on top of $T.rc$'s snapshot.
...
We achieve this by requiring that, whenever an object $o$ that is base object of some views $v_1, ..., v_k$ is updated in $T$, the respective updates for views $v_1, ..., v_k$ to be included in $T$.
To be precise, for each view $v_i \in v_1, ..., v_k$ the update must set the view's state back to $v_i = fun_{(v_i)}(o, ...)$.
...
Updates received are also applied according to causal order, keeping the replicas consistent and ensuring state convergence.
%Do I need to mention transactions here?
%Or maybe that we use vector clocks inspired in clock-si?

In this section we discuss how objects are partitioned across PotionDB instances and how our replication algorithm takes advantage of this partitioning.
...
Buckets are the unit of partitioning - objects with the same bucket value will be replicated by the same set of servers.
Each server replicates a set of buckets.
...
To exemplify, consider the e-commerce example in Section \ref{sec:introduction}.
...
\begin{compactenum}
	...
	\item Configure each server to replicate the continent it is located in + another continent. Do this in such a way that each continent is replicated in, at least, two datacenters;
...
When a transaction commits localy, it is queued for replication.
...
Note that $T^{bkt}$ maintains the order of updates, that is, for any pair of updates $u_1$, $u_2 \in T^{bkt}$, if $u_1$ (resp. $u_2$) comes before $u_2$ (resp. $u_1$) in $T$, then $u_1$ (resp. $u_2$) comes before $u_2$ (resp. $u_1$) in $T^{bkt}$.
This ensures that updates for the same object are applied by causal order and, thus, the correct final state is reached in every replica. %Should I had used "happens-before" symbol instead of "comes before"? I did not since we usually used happens-before between different transactions (thus denoting different clocks)
...
For a given transaction $T$, all of its $T^{bkt}$ can be sent with an arbitrary order.
...
We assume an underlying communication mechanism that ensures causal order delivery, ensuring that all servers receive transactions from a given server according to their serialization order in said server.
We also assume some mechanism that allows knowing which buckets each server replicates, in order to only send to each server the $T^{bkt}$s relevant to the receiving server.
...
Assume $S_1, ..., S_n$ is the list of PotionDB servers and $S_i, S_j \in S_1, ..., S_n \land i \neq j$.
$\forall k \in (1, ..., n \setminus i)$, server $S_i$ has a queue $q_k$ which holds the transactions received from $S_k$, by the order received.
A transaction $T$ on the head of a queue is only applied if T's clock is $\leq$ than $S_i$'s clock.
Note that transactions from different servers can be applied by any order if they are concurrent.
%And, for two transactions $T_1$, $T_2$ from different replicas that are not concurrent (let us assume $T_1 \prec T_2$), 
If they are not, e.g. if for two transactions $T_1$, $T_2$ from different replicas we have $T_1 \prec T_2$, then $T_1$'s clock will not be $\leq$ $S_i$'s clock until $T_2$ is applied.
Thus, $T_1$ will be applied after $T_2$, maintaining causality. %I do not know if this explanation is generic enough - the idea here is that, for transactions causally related from different replicas, they will be applied with causal order, as their clocks will enforce so (we cannot apply a transaction until the replica's clock reflects all preceding transactions).
%TODO: Need to define the "replica's clock" somewhere. Likely on transaction algorithm
...
We use RabbitMQ as follows. We run a RabbitMQ instance alongside each PotionDB server (replica).
When a replica starts, it contacts other server's RabbitMQs and subscribes the topics matching the buckets replicated by the replica.
...
Updates are sent in group to RabbitMQ, one group per bucket of a transaction.
The group's topic is set to the value of the bucket.
This allows RabbitMQ to later forward the updates to each interested PotionDB instance, ensuring each PotionDB receives only the relevant buckets and maintaining the transaction order.
The receiving replica rebuilds the transaction as it receives the buckets.
The end of a transaction is signalized either by the receival of another transaction from the same server, or by a special message in the case of being the last transaction replicated so far.
...
\begin{compactenum}
	...
	\item \label{repl_sol_2} Replicate both base object and view updates together. 
...
It would also require propragating all the base objects' updates (and possibly current state), which is undesirable, as it would defeat the purpose of partial replication.
Both requirements are undesirable.

Possibility \ref{repl_sol_2} faces different challenges.
Namely, 
\begin{enumerate*}[label=(\roman*)] 
	\item \label{item:view_challenge1}source replica may not replicate a view but may replicate some of its base objects;
	\item \label{item:view_challenge2}receiving replica may replicate a view but not some of the updated base objects.
\end{enumerate*}
Our solution must account for both challenges.
...
Challenge \ref{item:view_challenge1} is addressed as every server with base objects relevant to some view will now also have said view and, thus, include the necessary view updates in the local transactions and replicate said updates.
...
\begin{enumerate*}[label=(\roman*)] 
	\item \label{viewProp1} a server replicating a view receives all view updates;
	\item \label{viewProp2} views are always consistent with their base objects in any given server;
	\item \label{viewProp3} the state of the view in different replicas eventually converges.
\end{enumerate*}

For property \ref{viewProp1}, assume all servers replicate all views.
When a base object is updated, its view is updated in the same transaction.
The whole transaction will be sent to replication, and split by buckets.
Because all servers replicate all views, all servers will receive the view's updates and all servers will send the view's updates generated by their local transactions.
Thus, all servers receive all of the view's updates.

Property \ref{viewProp3} follows up from the propositions above.
Given all servers receive all updates and they are applied by causal order, and concurrent operations are commutative (views are CRDTs), all replicas will converge when the set of updates applied is the same.
...
Thus, $S_1$ replicates the buckets of $o_1, ..., o_k$ and as such will receive all updates for $o_1, ..., o_k$.
...
Thus, views and their base objects are kept consistent.
Note that $S_1$ does not replicate $o_k+1, ..., o_n$, thus consistency between those base objects and their views does not apply for those objects.
...
Our exact requirement is for view $v$ to be replicated in every server that replicates any of the objects in $o_1, ..., o_n$.
...
Cabrita et. al \cite{Cabrita17Nonuniform} introduces the concept of non-uniform distributed system.
...
Non-uniform replication allows to save both replication, processing and storage costs, as not all updates for a given object $o$ need to be replicated and applied everywhere.
...
We leverage on non-uniform replication, namely on non-uniform CRDTs (NuCRDTs) to provide materialized views with space-efficiency.
The savings on space and operation processing can be quite considerable, as in some scenarios most data is not relevant for a query.
E.g., in an global e-commerce scenario, consider the query "top 10 most sold products worldwide".
Even if there are hundreds of thousands of products, only a small subset of entries needs to be replicated everywhere to correctly reply the query, as the vast majority of entries will never be relevant.
In PotionDB, we support many NuCRDTs such as max/min, average, topK (with removals) and topSum.
We extended some of these designs for better expressivety.
...
Our topSum also supports decrements, allowing support for dynamic leaderboards where points can be added or subtracted whenever necessary.
...
In the latter case, a new transaction is created that holds the newly relevant operations that were generated by any of the NuCRDTs that were modified in the remote transaction being applied.
...
This new transaction may potencially make more operations relevant, however it is ensured eventually the observable state will converge.
...
Forward requests are executed under RC, thus different versions may be read.

We believe for most use cases, good data locality when doing partial replication, alongside the usage of views, will make needing to access other servers a very sporadic event.
We note that data which may be revelant everywhere can still be replicated in every server if so is desired.

\section{Designing views for applications}
\label{sec:views_for_apps}

%I think it needs a different motivation
In order to evaluate PotionDB's performance, as well as demonstrate the expressivity of PotionDB's views and objects, we implemented TPC-H's data model, as well as all proposed updates and a subset of the proposed queries.
In this section we discuss the experience of adapting a SQL-oriented database scenario like TPC-H to PotionDB, focusing on view design and limitations of our solution.
...
\begin{compactitem}
	...
	For orders and partsupply, we select, respectively, the customer's and supplier's region.
	...
	Our analysis into the dataset shows this happen frequently.
	\item Lineitem: associated to both the customer's and supplier's region, as this information is relevant for both customer and supplier.
\end{compactitem}

We highlight that views are fully replicated (they have their own bucket).
Thus, queries that access worldwide data, which are frequent in TPC-H, can still be served efficiently.
...
E.g., each customer is represented by a map, with the map's key being the customer's key.
...
Buckets are associated to each object as described in the previous section. %Is this phrase necessary?
...
PotionDB's view API (Section \ref{subsec:viewapi}) and its similarities with SQL make this challenge similar to the one faced when using relational databases.
PotionDB automatically generates the necessary objects and update rules for the view(s), ensuring the view is correctly updated when any base object of the view is inserted/updated/deleted.
Such inference takes out a considerable burden from the application developer.

The key point for specifying a materialized view is to determine the information needed to answer the query.
To illustrate the process, consider query 3 (Q3) of TPC-H \cite{tpch}, defined in Figure \ref{fig:q3} \footnote{For simplicity, we assume data is not normalized. Thus, some fields from customer and order tables are assumed to be available on lineitem.}:
...
For the view, we will want to select the same fields as in the query, so that all information necessary is in the view.
...
The \emph{from} and \emph{order by} clauses will also be the same. %Do I need to justify this?
Since the quey only needs to list the top 10 rows, we add a \emph{limit} 10 clause.
Finally, we need to modify the \emph{where} clause to add restrictions to DATE - this query parameter only varies between 1995-03-01 and 1995-03-31, so we add ``and 1995-03-01 $<=$ DATE $<=$ 1995-03-31''.
...
No restriction is needed for SEGMENT, as all valid values for SEGMENT are to be considered for the view.
Finally, the group by cause is not needed for the correct generation of either the view or its updates.
...
\begin{figure}[h]
	...
	\hphantom{SELECT }orderdate, shippriority \\
	FROM lineitem \\
	WHERE mktsegment = [SEGMENT] and orderdate $<$ [DATE] and shipdate $>$ [DATE] and 1995-03-01 $<=$ DATE $<=$ 1995-03-31\\
	ORDER BY revenue DESC, orderdate \\
	LIMIT 10 \\
...
\emph{Order By} specifies how the elements in the top will be sorted - first by revenue, then orderdate.
...
The parameters of a query, [SEGMENT] and [DATE], in practice represent different variations of the query.
...
The order is the same as the order by which the parametes appear in \emph{Where}.
Executing the right variation of the query is as simple as building the key and doing a direct get on the database.
 ...
 With both of the above, PotionDB concludes that to support Q3's materialized view a TopSum CRDT should be employed for each pair of (SEGMENT, DATE).
 ...
 That is, we support parameters whose set of possible values is not known or determined beforehand and remain efficient by only initializing objects as necessary.
 ...
 %TODO: I think our view API and automatic generation of objects/updates is a contribution! Need to add that.
...
After determining the objects used to represent the view, PotionDB generates two triggers that will be used to update those objects.
One trigger corresponds to new objects or updates of objects relevant to the view, while the other for removals of said objects.	%Is this safe to say? I don't know how updates would do actually.
The respective trigger is fired when an object relevant to the view is added/updated/removed, generating the necessary updates for the view.
...
The set of relevant objects for the view is determined by the \emph{from} clause - only objects with the container specified activate the triggers.
The conditions in the \emph{where} clause further restrict which objects may generate updates for the view.
...
For the rest of the explanation, assume \emph{view} refers to all possible variants generated by the parameters, \emph{subview} refer to one of the variants.
...
Testing $=, <, >$ and $!=$ between two fields or between a field and some constant value is straightforward. 
...
Note also the matching of fields in select to the view is also straightforward - this was already necessary to do previously to determine the CRDT(s) to represent the view.
In the example of Figure \ref{fig:q3_view}, it is known that a new lineitem is an increment to a TopSum, an update a increment or decrement depending on the change, and a remove a decrement.
This is known as it was previously determined that the view is represented by a TopSum, due to limit and the existence of the sum aggregation.
...
\begin{table}
	...
	\end{tabular}
\vspace{1em}
\captionof{table}{Updated views. Note that none of the other pairs of (mktsegment, date) get updated.}
...
%Note: We do support a very basic form of joining - must be a 1:1 match of keys.
...
Furthermore, they are all supported by manually supplying the triggers to the database, which also goes around the limitation of joins.
%Thus, PotionDB's datatypes are expressive enough to fully support all TPC-H queries with efficient views, demonstrating PotionDB's adequaty for OLAP

Finally, triggers raise security and safety concerns due to the usage of custom code. However this is a shortcoming of our implementation and not of triggers themselves - it would be possible to use an existing data manipulation language (e.g., \cite{oracleTriggers}) instead of custom code.
...
\begin{compactitem}
	...
	This implies that an update to one single object may have to update multiple views. 
	...
	Thus, this required a different approach to querying a TopK CRDT, requiring two views: one with the top, and another with a counter maintaining the actual value of said factor.
	...
	\item Query 15: this query not only mandates the usage of a TopSum CRDT, it also has an interesting query pattern - only one entry of the top is returned except when multiple entries have the same max value. %Thus it's interesting because in theory we could keep less data in each server.
	...
	Second, the TopK is expected to have relatively few entries, as the restriction is quite limiting - thus, most updates to base objects won't generate any update for the view.
\end{compactitem}

Despite only having implemented 6 of TPC-H's queries, we highlight all were analyzed.
...
Thus, PotionDB's datatypes are expressive enough to fully support TPC-H queries' efficiently, demonstrating PotionDB's adequaty for OLAP scenarios. %Maybe replace OLAP with analytical.
...
\begin{itemize}
	...
	\item Each PotionDB instance replicates the data concerning one continent (i.e., one replicates all data relevant for Europe; another replicates for Asia, etc.)
	Data that is not location based, like products, are replicated in all servers.
	...
	\item Each client selects one server to execute queries upon, and only contacts said server. In the case of local view and single view PotionDBs, if a server cannot satisfy a query with its data, that server contacts other servers in order to reply to the query.
...
%	\item Each PotionDB instance replicates the data of a continent, e.g., a server replicates all data concerning Europe while another keeps the data of Asia. Data that is not location based such as products is replicated in all servers, as are indexes.
...
Finally, after all data has been loaded into the servers, multiple clients per client node are started to execute queries and, possibly, updates, with the query/update rate depending on the test.
...
This occours due to the fact that queries are served entirely by views, which are all in a single server - thus, in a query-only scenario, one server gets all the load, instead of it being distributed by 5 servers.
...
However, updates in a transaction must lock the partitions they are involved and do a 2-phase commit, in order to ensure the states evolve sequentially in a replica even when concurrent transactions are issued.
...
It is noticeable that updates, even for low values, have higher latency than queries - this is mostly due to the fact that update transactions have more operations than queries.
...
%Graph with bars, showing the "max" performance of TopK and TopSum in some different scenarios (different top sizes and update rates). Maybe stack TopK and TopSum, and use as groups the same Top size?
\begin{figure}
	\centering
...
\begin{figure}
	...
	\caption{[OLD] PotionDB performance when batching queries. %Note that latency is in logarithmic scale.}
	}
	\label{fig:ops_per_txn}
\end{figure}
...
%Graphs:
%TopK with 5 servers in which we vary the number of elements on top;
%Maybe some "bar" graph for updRate + servers? The idea is, each bar = number of servers. Then, in each bar, there are the multiple update rates. Scale may be an issue, as the max amount of ops/s is 11M, and we can go under 1M in some scenarios.


%\andre{Outdated. If this is still necessary, we need to define precisely what we want to showcase here.}
%
...
Geo-distributed scenarios imply high latency when acessing far away datacenters, which is required for strong consistency \cite{chronocache, slog, cops, eiger}. 
...
Partial replication allows to reduce storage costs and can even improve a system's performance (e.g., due to less operations being applied on each server) \cite{spire, optimisticPartial, coda}, and are thus of interest for geo distribution.
The partitioning of the data must be carefully done by considering where each object is relevant, as otherwise clients may need to access far away servers and thus experience high latency \cite{spire, optimisticPartial, coda}
However, one common problem with partial replication is that, sometimes, it is necessary to access data sharded among different datacenters \cite{spire}.
...
PotionDB supports both Parallel Snapshot Isolation (PSI) \cite{walter} and Read Commited (RC) as its consistency models.
