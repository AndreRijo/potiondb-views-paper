...
CREATE VIEW (MonthlyTopSales, views) WITH 
		SaleDate.Year == ANY (SELECT DISTINCT SaleDate.Year FROM sales), 
		SaleDate.Month == ANY (SELECT DISTINCT SaleDate.Month FROM sales) AS
...
We have analyzed how to define views in PotionDB for the the 20 queries in TPC-H, with different views using different
underlying CRDTs.
...
The view presented in Figure~\ref{fig:viewtopsales} is an example of such de-normalization, with
the product name (\texttt{prodname}) stored in the sales record, instead of the identifier of the product.

\begin{figure}[t]
\small{
\begin{lstlisting}[language=SQL]
CREATE VIEW (Q5, views) WITH REGION == ANY (SELECT DISTINCT region FROM regions) AS
...
This contrast with the top sales presented in Figure~\ref{fig:viewtopsales}, that uses a
...
to all replicas the updates for elements that may belong to the top-K.

Finally,  some view are supported by top-K objects. Figure~\ref{fig:q18_view} presents the view to 
...
from 312 to 315. In general, it is common for application interfaces to limit the queries a use can perform, 
...
WHERE N_Name == [NATION]
\end{lstlisting}
	\caption{Important stock identification query (Q5).}
	\label{fig:q5_view}
\end{figure}

In general, it is possible to circumvent the limitations by defining additional views.
For example, TPC-H query 5 finds the suppliers stocks in a given nation that represent a significant percentage of the total
...
maintains the top parts for each nation. For reaching the result of TPC-H Q5, the application needs
