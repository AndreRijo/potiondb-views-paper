...
Views are kept up-to-date automatically without requiring any extra coding or update logic from 
...
than alternative approaches where the view is computed in a single data center, while being
able to keep consistency between the base and view data in every replica.

In this paper we make the following contributions:
%\begin{enumerate*}[(label=\roman*)]
\begin{compactitem}
	\item design of a geo-replicated key-value store  with support for partial replication
	and views over partially replicated data (Section~\ref{sec:transactions}); 
...
%	\item a language inspired in SQL for view specification in key-value stores, with view maintenance code automatically inferred from the specification;
	 \item implementation and evaluation of the proposed approach with micro-benchmarks
	 and TPC-H (Section~\ref{sec:evaluation}).
\end{compactitem}
...
An application issues interactive transactions, by executing operation \code{begin(clk)},
...
\end{inparaenum}
Operations defined in each object are type-specific - e.g. a set object has a \code{contains(e)} operation to check
...
PotionDB also allows an application to issue an one-shot
%~\cite{??????} 
...
The operation to create a view receives a view specification defined in a language based on SQL~\cite{sequel}.
...
\qcr{CREATE VIEW} specifies in which key (prefix) and bucket the materialized view will be stored.
...
As usual in SQL, it is possible to define aggregations over groups with the \qcr{GROUP BY} clause,
...
In the example, we have variables \qcr{YEAR} and \qcr{MONTH}, 
which will lead the system to maintain
for each pair (year, month) that exists in the database, the top 10 sales.
...
Given a view definition, PotionDB automatically infers the objects to be used to store the view contents and
...
row and the ordered value used for selecting the top rows.
...
row, with the operations of the data type associated with the row element. \andre{In the implementation, a Top-K entry is a key, value and "an array of bytes" (so, not other CRDTs inside the Top-K). Should we change the text to reflect that?}
...
cases in which a replica may be exposing incorrect results in practice. In our example, the maximum NuCRDT
maintains the two largest elements. Therefore, if the maximum element
is deleted, replicas still have the new maximum. 
Doing this is straightforward for all NuCRDTs we have defined, besides the top-K counter. 
...
As mentioned in the previous Section, due to the way NuCRDTs work, they may temporarily expose 
...
this is expected to be a rare situation. \andre{Shouldn't we mention again we have mechanisms to make this situation even more rare to happen? (e.g., maintaining a top of double the size.)}
...
In NuCRDTs, the execution of an operation $o$ may make a previous operation  $o_{prv}$
...
%
The second case happens when the now relevant $o_{prv}$ operation was executed in  
...
the effects of $o_{prv}$. These extra effects are remotely propagated 
%to other locations 
along with the 
...
In this section we evaluate PotionDB's prototype, aiming at answering the following questions: 
\begin{enumerate*}[label=(\roman*)]
...
Out prototype was implemented in Go and uses Google's protobufs to serialize/deserialize all data transmitted.
...
This results from the fact that all queries are local, while in alternative approaches, global queries need to access data from 
remote replicas. In this case, servers end up consuming most of the time managing communications with other replicas, leading not only
to higher latency but also to lower throughput. 
...
For the global query, the results are similar to the complete workload, with the main difference an increase in latency and reduction in the throughput for the \textit{Local} configuration. This is explained by the fact that in this case, all queries need to contact remote servers and results do not include the effects of operations that execute locally.
...
\begin{figure}
	...
	\begin{subfigure}{.52\linewidth}
		...
		\caption{\textbf{P}otionDB and \textbf{S}ingle configurations.}
...
\textit{Local} throughput improves as the ratio of updates increase.  This results from the fact that we 
...
Additionally, update transactions do not need to contact other servers - view updates are propagated asynchronously -,  
...
refer few regions.  This leads to a slightly lower average latency with higher update rates.
...
This section studies how different views behave and scale for different configuration. 
To this end,  we study the behavior of the underlying CRDTs used for supporting different types of 
view: simple limits (Top-K), limits over aggregations (Top-K counter), sum/count (Counter) and average (Average) aggregations.

These experiments were performed using micro-benchmarks, where the underlying CRDTs are 
...
Figure \ref{fig:topKVSTopSum} shows that both TopK and Top K Counter maximum throughput is similar, which
shows that the mechanism used for deciding when it is necessary to propagate updates for Top K Counters 
perform well. When considering different update ratios, for small tops 
...
Careful data partitioning is essential to avoid clients having to contact far away servers, thus experience high latency~\cite{sipre, optimisticPartial, coda}.
...
PotionDB's transactional causal consistency, alongside a rich library of data types, view API, and usage of CRDTs to automatically handle concurrency conflicts, ease the programmer’s complexity on developing distributed applications 
%using PotionDB 
