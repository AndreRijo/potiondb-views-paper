...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\EndFunction
		\Function{canCommit}{$\mathit{txId, maxTs}$}
			\State $\mathit{prep_i}$.\Call{remove}{$\mathit{txId}$}
			\If{$\mathit{prep_i}$.\Call{isEmpty}{}() \&\& $\mathit{hold_i}$.\Call{min}{}().$\mathit{ts} < \mathit{maxTs}$}
				\State \textbf{return} false
			\EndIf
			\If{$\mathit{prep_i}$.\Call{min}{}().$\mathit{ts < maxTs}$ $\|$ ($\mathit{hold_i}$.\Call{notEmpty}{}() \&\& $\mathit{hold_i}$.\Call{min}{}().$\mathit{ts < maxTs}$)}
				\State \textbf{return} false
			\EndIf
		...
		\Function{checkHoldCommits}{}()
			...
			\If{$\mathit{prep_i}$.\Call{isEmpty}{}()}
				\While{$\mathit{hold_i}$.\Call{notEmpty}{}()}
					\State $\mathit{txId, cc = hold_i}$.\Call{removeMin}{}()
					\State \Call{applyCommit}{$\mathit{txId, cc}$}
				\EndWhile
				\State \textbf{return}
			\EndIf
			\State $\mathit{minPrep = prep_i}$.\Call{getMin}{}().$\mathit{ts}$
			\For{($\mathit{txId, cc}$) in $\mathit{hold_i}$}
				\If{$\mathit{cc < minPrep}$}
					\State $\mathit{holdi}$.\Call{removeMin}{}()	\Comment{Same as the currrent value of $\mathit{txId, cc}$}
					\State \Call{applyCommit}{$\mathit{txId, cc}$}
				\Else
					\State \textbf{return}
				\EndIf
			\EndFor
		\EndFunction
		\Function{applyCommit}{$\mathit{txId, rc, maxTs}$}
		...
		\EndFunction
		\Function{update}{$\mathit{txId, upd}$}
			\State $\mathit{txUpds[txId]}$.\Call{append}{$\mathit{upd}$}
		\EndFunction
		\Function{applyRead}{$\mathit{txId, rc, read}$}
		...
		\EndFunction
	\end{algorithmic}
	\caption{Commit and read protocol execution on S\textsubscript{i}'s partition sh\textsubscript{j}}
	\label{alg:commit_part}
	%IMPORTANT NOTE: There’s a situation @ applyRead that is not addressed in neithe the pseudo-code nor the last version of the code (it is addressed in the one that was used for the paper’s evaluation however. Also addressed in the text description in the paper). To more easily explain this, consider:
