...
Namely, we try to assess the advantage of all replicas having views spanning data partitioned accross multiple replicas, instead of each replica having an incomplete view with only its local data (local PotionDB).
...
\begin{itemize}
	...
	%\item How does PotionDB compare with the other mentioned solutions, on a query per query basis? Namely, how does the locality and size of data affect the performance difference?
...
To evaluate the benefits of having indexes replicated in every replica, we compare PotionDB with the local and single versions, namely in terms of queries per second.
...
This can be explained as all queries only need to consult the indexes, which due to those being all in one server, implies that the load is all directed to one server instead of being split accross five.
...
We now evaluate how much of an impact on PotionDB's throughput has executing updates alongside queries.
...
Thus, we now evaluate the effect observable on PotionDB's throughput if multiple queries are included in the same transaction.
...
This may be due to the fact that a lookup operation in a set CRDT executes so quickly that most of the overhead may be in communication between threads or on converting data to the wire, thus not taking much benefict from the CRDTs being split accross different partitions.
...
%It is expectable for Local PotionDB to have worse performance, as most TPC-H queries concern data that is replicated accross multiple replicas, which implies that clients need to contact multiple replicas to answer a single query in the case of local views.
%
%\begin{figure}
%	\centering
%	\begin{subfigure}{.5\linewidth}
