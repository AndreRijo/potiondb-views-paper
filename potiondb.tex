...
to create views. As it is common is key-value stores and document databases, buckets have no associated
...
Figure \ref{fig:viewSQL} shows the specification of a view that maintains the top 10 sales for
each month. The create view specifies in which bucket and object the the materialized view object will be stored.
...
When a transaction commits locally, the TM sends the transaction to the replicator for being replicated.
it is queued for replication.
Periodically, our replication algorithm collects queued transactions and sends them asynchronously to other replicas.


When a transaction 

Considering that there is a replica for the EU online store, that replica would replicate the 
buckets 
 would 
be defined 

 would include  for the custo 
