...
data type, i.e., any object type can be stored in any bucket although it is common for application to 
...
Figure \ref{fig:viewtopsales} shows the specification of a view that maintains the top 10 sales for
each month. The create view specifies in which bucket and object the  materialized view object will be stored.
The \qcr{FROM} clause specifies which container(s) are used in the computation of the view, and the \qcr{SELECT} specifies
...
a transaction always executed in a causally consistent snapshot.

Transactions can execute concurrently, with each replica
executing transactions according to a different valid
serialization.
To guarantee state convergence, we use CRDTs~\cite{crdts,walter},
...
Third,  the Replicatior ensures that committed transactions are replicated asynchronously to other PotionDB instances.
...
Each value of the objects used in a view will be a map with multiple columns, one for each
of the attributes specified in the  \emph{select} of the view definition. 
For a given view definition, one or multiple objects can be defined, depending on whether a 
view has parameters or not. If it has parameters, the values of the parameters will be used in the name
of the view. object
In the example of the top sales presented before, one top-K counter
object will be created (as needed) for each month of the year, and its PotionDB key will include
the name of the view and the month and year.   

\noindent
\textbf{Generated triggers:}
After determining the objects used to represent the view, PotionDB generates a trigger to to update 
the contents of the view, as base objects are created, updated or deleted (referred simply as updates). 
The trigger will be set for the container (or bucket) specified in the \emph{from} clause of the view
definition.
...
As a view may be composed by multiple objects, the first thing is to 
determine which view object must be updated - this is achieved from the values in the updated base 
object.



used for defining the name of view object.

. This is done by using the values of the base object
to deter  
...
An update in a base objects may lead to a new new view objects


When a base object is created, it may 

An update

When a base object is updated, this update may lead to a n 

Given an update to a base object, the 
