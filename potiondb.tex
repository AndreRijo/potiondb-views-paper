...
\newcommand\vldbavailabilityurl{} %TODO
...
specifying view,s  a view that includes variables will lead to multiple objects,
...
an application developer can define PotionDB object to maintain the view's data and the
...
In a query-only scenario (P 0\% and L 0\%), there is no observable impact. %And I suppose this makes sense for as long as there's enough RAM available.
Surprisingly, even with a modest update ratio (P 25\%), PotionDB still keeps up its total throughput. %I believe with batching of queries, the throughput would decrease as the number of views increase. But for now, the major cost is on communication and locks in TM.
...
%TODO: Worth discussing: The data used for the query and update graph are different: for the query graph, there's no updates, while for the update graph, there's one client executing only updates and a varying number of query-only clients. My goal was to first focus in a query-only scenario (the best for Postgres), and then show how bad updates are for Postgres (and hence show only the throughput of updates). However this means I do not show the query throughput when executed with updates... is that ok?
...
\begin{figure}
	...
	\begin{subfigure}{.49\linewidth}
		...
		\caption{Query throughput with no updates.}
		\label{fig:postgres_query}
		\vspace*{0.7em}
	\end{subfigure}%
	\hspace*{0.3em}
	\begin{subfigure}{.47\linewidth}
		\includegraphics[width=1\linewidth]{postgres/postgresUpdateOnly}
		\caption{Update throughput with varying query clients and a single update client.}
		\label{fig:postgres_update}
	\end{subfigure}%
	\vspace*{-0.65em}
	\caption{Postgres vs PotionDB. P, S and PG stand for, respectively, PotionDB, Single PotionDB and Postgres. All represents all selected queries, SubQ consists of Q3, Q5, Q14 and Q18, SubU consists of Q11, Q14 and Q18.}
	\label{fig:postgres}
	\vspace*{-0.2em}
\end{figure}

%TODO: I think we didn't define anywhere what was our set of selected queries. Yet we refer to this 'set of selected queries' a few times through the evaluation section.
...
Q15 is not efficiently supported in PostgreSQL as pg\_ivm does not support both \emph{order by} nor aggregations of aggregations, thus to answer this query extra computation is required and multiple rows are scanned. %It needs to calculate a "max" manually.
...
However the max throughput reduction is much higher in Postgres (96.8\%) than Single (28.5\%) or PotionDB (25.5\%). %Maybe with raw numbers the difference looks more significant? Postgres: 320k -> 10k query/s. Single. 288k -> 206k. PotionDB: 1371k -> 1044k.
