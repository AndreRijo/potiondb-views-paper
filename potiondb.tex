...
In this section we evaluate the performance of PotionDB's prototype.
Namely, we try to access the advantage of having, in each server, views that span data partitioned across multiple servers.
...
\begin{compactitem}
	...
	\item \emph{Local view PotionDB} or \emph{Local PotionDB}: each server contains views concerning only local data - e.g., on an European server, $v$ would only consider orders from European customers.
	...
	\item \emph{Single view PotionDB} or \emph{Single PotionDB}: only one server has all views (i.e., like a central server), but views are complete.
...
Specifically, while we keep all tables of TPC-H and use data generated by the dbgen tool \cite{tpch}, we only use the subset of queries described in Section \ref{subsec:implementedQueries}.
Furthermore, for each table, we only keep the necessary columns to answer said queries.

Of the queries selected from TPC-H, queries 3, 11, 14, 15 and 18 concern world-wide data, while queries 5 and 11 concern, respectively, data of one continent/country.
We name both groups of queries as, respectively, ``global queries'' and ``local queries''.
Thus, whenever a view reflects worldwide data or local data we designate them as, respectively, global view or local view.

Overall, our test environment has the following configuration:

\begin{itemize}
	\item Five PotionDB instances, with one instance per node.
	Another five nodes are also used to execute PotionDB's clients. %Maybe mention grid.
	\item Each PotionDB instance replicates data concerning one continent.
	Products, which are not location based, are replicated everywhere.	%May need to explain lineitems replication here.
	View replication works as previously described.
	\item All nodes contain 2x Intel Xeon Gold 6130 with 192 GB of ram, interconnected with a 10Gbps Ethernet network.
	\item Latency was imposed between servers, in order to simulate a geo-replicated scenario.
	...
	For latency, we use as reference the latency between AWS instances as measured in \cite{AWSLatency} on February 2022.
	The following regions were selected: EU (Paris), US (N.Virginia), Canada (Central), Asia Pacific (Tokyo) and Middle East (Bahrain). The highest latency measured is between Canada and Middle East at 204ms.
	...
	\item Each client selects one server to execute queries upon and interacts only with said server.
	There is no latency added between clients and servers, in order to simulate locality.
	For local and single view PotionDB, if a server cannot satisfy a query fully with its data, said server contacts other servers in order to reply to the query.
	\item Google's protobufs are used to serialize/deserialize data in client-server and server-server communication.
	PotionDB's protobuf interface is an extension of AntidoteDB's protobuf interface \cite{AntidoteDB}.
...
After the loading phase is complete, the clients connect to the servers and start executing queries and, optionally, updates, with the query/update rate depending on the test.

In our experiments we use as metrics the amount of queries per second (queries/s) achieved by all clients aggregated and the average latency of each transaction (including RTT).
In tests with both updates and queries, we instead use operations per second (ops/s).
Note that all updates for views are not counted towards throughput.
...
%Include the below in the Scenarios subsection.
...
\begin{enumerate}
	\item \label{enum:q1} In a partially, geo-replicated scenario, what are the benefits of having views of worldwide data in every server? Namely, how does that compare to views of only local data (Local PotionDB) or a single, central server, responsible for providing global views of the data (Single PotionDB)?
...
For questions \ref{enum:q1}, \ref{enum:q2}, \ref{enum:q3}, TPC-H's benchmark and dataset is used, while for question \ref{enum:q4} a micro-benchmark is used instead (we leave details for Section \ref{subsec:microbenchmarks}).
...
\begin{figure}
	...
	\caption{Query-only performance of PotionDB}
	\label{fig:global_local_single_tc}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=1\linewidth]{singleQuery/all_queries_tc_singleServer_latencies}
...
Figure \ref{fig:global_local_single_tc} shows the results of running all selected queries (with no updates) of the TPC-H benchmark for PotionDB, Local PotionDB and Single PotionDB.
Each client executes one query per transaction.
Results are shown in query/s versus average latency/query.

PotionDB is able to achieve up to 1.5M queries/s, with a low average latency varying between 0.15ms and 0.6ms, depending on the number of clients.
...
PotionDB's performance can scale further if multiple queries are executed per transaction, as discussed in Section \ref{subsec:???}.

On the other hand, Local PotionDB is highly affected by the latency between servers.
On average, each client request takes 140ms to complete.
The high latency is due to global queries (2/3rd of the query types) requiring information from all regions, thus a client's request for one of those queries needs to be forwarded to all servers.
The other 1/3rd are local queries which are quickly executed with low latency.
...
Finally, Single PotionDB's max throughput is approximately 1/5th of PotionDB's.
This is expected as queries are entirely served by views, which are all in only 1 out of the 5 servers - thus said servers gets all the load, instead of it being evenly distributed.
The latency may, at a first glance, seem unexpectedly low however, given that 80\% of the clients suffer from latency due to their requests having to be re-forwarded to the server with views.
Figure \ref{fig:single_tc_latencies} shows the average latencies of clients grouped by the server they are connected to.
In practice what happens is that the throughput is mostly consumed by the clients directly connected to the view server, as other clients spend most of their time waiting for RTT - thus the average latency of all is low.
Thus, Figure \ref{fig:single_tc_latencies} gives a more accurate representation of the latencies experienced by clients in Single PotionDB.

In conclusion, having global, complete views replicated in every server allows all clients to achieve low latency and high throughput in PotionDB, while Local and Single PotionDB suffer from high latency for, respectively, in all global queries for all clients or in both local and global queries for clients far away from the server with views.
...
TODO/NOTE: Not down from here afterwards

NI have to do these two graphs (I have the necessary data however).
