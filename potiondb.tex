...
selling products overall and for users that bought some other product).  For supporting the latter, %operations,
...
Figure \ref{fig:topSalesSQL} shows the specification of a view that maintains the top 10 product with most profit in sales for
each month. %The create view specifies in which bucket and object the  materialized view object will be stored.
The create view specifies in which key (prefix) and bucket the materialized view will be stored.
...
restrict the number of elements with a \qcr{LIMIT} clauses, which can be ordered using a \qcr{ORDER BY} clause.

In recurrent queries, it is common that a generic query is instantiated with different values. 
To support this, our language allows to include variables in the view definition, specified using 
square bracket. 
In the example, we have variables \qcr{YEAR} and \qcr{MONTH},  which will lead the system to maintain
...
to a valid serialization, objects will have to the same state (by relying on the
deterministic conflict resolution policies defined in the CRDT user for each database object).
...
Thus, we assume PotionDB instances are spread at different locations across the globe (Figure \ref{fig:arch}).
...
First, the TransactionManager is responsible for receiving and replying to client's requests.
	It coordinates transactions, implementing a transactional protocol (details shown in Section \ref{sec:transactions}) %which ensures
...
row, with the operations of the data type associated with the tow element.
...
In this section we start by discussing how view are supported in PotionDB and proceed to 
discuss how experience in defining views for the queries of the TPC-H benchmark~\cite{tpch}. 
...
specifying view,  a view that includes variables will lead to multiple object,
one for each possible value of the variables. 
In the example of the top sales presented in Figure~\ref{fig:viewtopsales}, one top-K counter
object will be created (as needed) for each month of each year, and its PotionDB key will include
the name of the view and the month and year.   The top-K counter will have a set
...
Aggregations defined in the views - e.g. average, maximum - are supported by using CRDT and NuCRDT 
...
TPC-H defines 20 queries and we have defined views for each of these queries. We now describe 
the lessons learned. 

First, PotionDB is a key-value store and does not directly support advanced database functionality,
such as joins. 


Given this, it was necessary  


we do not support joins in our view definition languages.  To make it possible to 
define views using our language, in most cases it was necessary to use a 

t was necessary to 


 our view definition language

having a non-normalized 

 a number of que views. 


Given a recurrent query, the application developer can specify one or more views that hold the necessary data 
to answer the query directly, without requiring to read multiple objects, do joins or other resource-consuming processing.
As in relational databases, the challenge is to specify the right views for the application's queries.
PotionDB's view API (introduced in Section \ref{subsec:interface}) and its similarities with SQL makes this challenge somewhat 
similar to the one faced when using relational databases.

When devising the views, an important aspect to consider is that some generic queries performed by applications
have parameters that are instantiated when executing the query. In the example of the top sales of 
Figure~\ref{fig:viewtopsales}, the application will typically use the query for a specific month 
(as specified in the TPC-H benchmark). 

 For example, the application will 
are param

 have parameters 
that will be instantiated when the query is performed by the application. 


For example, the monthly top sales view 
presented in Figure~\ref{fig:viewtopsales} maintain the can be used to access the top products in each month, but the application
will typically perform the query for a specific month.
...
\end{figure}
 ...
 queries have parameters that  parameterized when 
...
PotionDB's view API (Section \ref{subsec:interface}) and its similarities with SQL make this challenge somewhat 
similar to the one faced when using relational databases.
...
Furthermore, our language can be extended with ease to support more operations on any supported datatype.
