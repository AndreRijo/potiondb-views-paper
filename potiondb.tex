 ...
 \acmSubmissionID{0} %TODO: Update this after submitting the abstract.
...
selling products worldwide and users that bought some product).  For supporting the latter, %operations,
...
Third,  the Replicator ensures that committed transactions are replicated asynchronously to other PotionDB instances.
	...
	The replication component is aware of each instances' replication scheme, ensuring each one only receives updates for objects it replicates. We detail this in Section \ref{sec:replication}.
...
Database systems can offer different levels of consistency, with systems usually labelled as either strongly or weakly consistent.
...
Causal consistency is a sub-form of weak consistency that alleviates this problem, however anomalies can still be observed even with cross-object causality \cite{cops, burckhardt2013understanding}.
...
Partial replication reduces storage costs and can potentially improve a system's performance, e.g., due to less operations being applied on each server~\cite{sipre, optimisticPartial, coda}, and is thus of interest for geo-distribution.
Careful data partitioning is essential to avoid clients having to contact far away servers and thus experience high latency~\cite{sipre, optimisticPartial, coda}.
However, queries concerning data sharded across data centers is challenging and slow, specially when trying to provide a consistent snapshot.
...
Marviq leverates on materialized views for handling range queries.
