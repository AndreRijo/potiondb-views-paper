...
\andre{This has some overlap with Section \ref{subsec:databasemodel}}
...
We consider a view to be any object for which it is possible to describe its state as being a funcion of object(s).
...
We now explain precisely which guarantees PotionDB provides under those two models.

To start with, we will consider only one server and the Snapshot Isolation (SI) model. 
...
Updates in a transaction are executed according to issued order.
...
Given said statement, along with our snapshot definition, we can state the following theorem:
...
Parallel Snapshot Isolation (PSI) is an extension of SI.
PSI allows for transactions executing in different servers to have different commit orders - that is, transactions can be concurrent when happening in different servers.
The implication of this is that transactions originated from different servers may have a different execution order from server to server.
Two transactions $T_1$ and $T_2$ are concurrent if $T_1 \not\prec  T_2 \land T_2 \not\prec  T_1$.

\andre{I don't know how to define mathematically PSI. Basically what I want to say mathematically is that a serialization is correct if the order with which the transactions are applied respects the happens-before relationship of transactions.}
