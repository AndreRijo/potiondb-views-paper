...
detailed in Section \ref{subsec:buckets}. An object has a $\mathit{key}$ inside the container (and bucket).
...
each month. The create view specifies in which bucket and object the the materialized object will be defined.
The \qcr{FROM} clause specifies which bucket(s) are used in the computation of the view, and the \qcr{SELECT} specifies
the contents of the view, which can include simple attributes of the selected objects or aggregations.
...
\paragraph{Transactional Causal Consistency}
