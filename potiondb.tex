...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\Function{startTransaction}{clientVC: vClock}
		...
		%Maybe on Sharding subsection I should mention we can get the shard of any object, update or read by doing ^{shard} (instead of just saying of any object)
			\State \textbf{send} \Call{update}{txId, o} \textbf{to} o$^{\mathit{shard}}$
		\EndFunction
		\Function{readOp}{txId: ID, readVC: vClock, o: ???}
			\State \textbf{send} \Call{read}{txId, readVC, o} \textbf{to} o$^{\mathit{shard}}$
		\EndFunction
		...
		\Function{globalClockUpdate}{ }
			\ForAll{T \textbf{in} ongoingTxs}
				...
				\EndFor
			\State vcG[j \begin{color}{red}(what is j?)\end{color}]  $\leftarrow$ \Call{max}{vcG[j], T.cc}
...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\Function{applyRead}{$\mathit{\textrm{Id } txId, \textrm{Clock } rc, \textrm{Read } read}$}
			\If{$\mathit{rc \leq vc_i}$ \&\& $\mathit{rc \geq objs[read.id]}$.\Call{lastVersion}{}()}
