...
\begin{compactitem}
	\item \code{begin(clk) $\rightarrow$ txId}: starts a transaction and returns an identifier (\code{txId}) to represent it. 
	The \code{txId} is then used to associate reads and updates to this transaction. 
	The \code{clk} is used to ensure applications see a casually evolving state of the database. We detail this further in Section~\ref{sec:transactions};
	...
	E.g., if $o^{\mathit{id}}$ is a set, then all its elements are returned; \carla{why not "if $o^{\mathit{type}}$ is a set"}
	...
	returning the result of the operation. E.g., if $o^{id}$ is a set, we could execute a $\mathit{lookup}(a)$ operation to check if the element $a$ is part of the set;
	\item \code{update(txId, id, op) $\rightarrow$ ok}: associates an update of object $o^{id}$ to the transaction \code{txId}.
	\textsf{op} represents the operation to be applied to modify the object's state.
	 Updates are only executed when a commit is issued. If $o^{\mathit{id}}$ does not exist, it is created during the commit;
	\item \code{commit(txId) $\rightarrow$ clk}: commits the transaction \code{txId}, applying all updates associated to it. 
	The clock generated by commit is returned for use in later transactions;
	\item \code{abort(txId) $\rightarrow$ ok}: cancels the transaction \code{txId}, discarding all updates associated to the transaction;
	\item \code{readTx(clk, read$^+$) $\rightarrow$ clk, value$^+$}: executes a read-only transaction with one or more reads; \carla{Is the read a sequence of read operations?}
...
The latter provides more flexibility for the application developer, as it allows for data to be read and then an action to be taken based on the result of said reads, 
all in the context of the same transaction.
...
CRDTs (Conflict-free Replicated Data Types) are designed to be used in distributed systems, ensuring the states of the replicas converge despite the occurence of concurrent updates.
Many CRDTs have been proposed, allowing for a rich library of objects such as counters, registers, sets, maps, topK (leaderboard). \carla{maybe topK needs to be further explained}
CRDTs support operations to directly manipulate the object's state, e.g., increments on counters, instead of direct assignements.
...
This avoids the need for ad-hoc mechanisms that are error-prone to compensate anomalies \cite{???}.\carla{check refs 17, 18, 22, 23 on the paper Composition in State-based
...
E.g., in a map, both single and group additions/removals can be executed, and one can read the whole map, check if a certain key exists or get the value of a key.
...
We also analysed the remaining queries and sketched solutions for implementing all remaining queries using PotionDB's objects and views, which demonstrates PotionDB's expressiveness. %Maybe I should say datatypes instead of "objects and views"?
...
Note that the API to read a view's state is the same as the one previously described to read other objects. \carla{Is is table 1. If so, add a link}
...
\begin{figure}
	...
	\begin{minipage}{0.19\textwidth}
		...
		\end{tabular}
		\vspace{1em}
		\captionof{table}{\textit{TopSales} materialized view. This view builds upon \textit{Sales} objects.}
...
The attributes for the view can potentially be aggregates (e.g., \qcr{sum}) of the objects' attributes.
Clause \qcr{WHERE} allows to define conditions that are used to decide whenever a given object is considered for the view or not, depending on certain properties of the object (e.g., only include European customers in the view).
Clause \qcr{ORDER BY} sorts the rows of the view given one or more properties of the view.
...
Any parameter of the intended query should be indicated between square brackets. \carla{Yes, an example is needed.}
%Should I give an example of what this is? By parameter I mean, e.g., queries that have many variants - e.g., asking the same query for different days of the week. The day of the week is a parameter of the query.
...
The \qcr{FROM}  clause then refers to one (and only one) of those containers. %Should I mention our restriction of only one container in some other way?
...
Figure~\ref{fig:topSalesSQL} shows a specification of a view which answers the following query: \emph{``Who were the top 10 most profitable customers on February? And how many products did those buy?"}
...
The \qcr{FROM} specifies that all objects belonging to the container \emph{Sales} should be considered, while \qcr{SELECT}  lists which attributes should be part of the view.
Several aggregation functions are supported: in this case, \qcr{sum} and \qcr{count} are used.
We also support \qcr{max}, \qcr{min} and \qcr{avg}.
The \qcr{WHERE} clause allows for only sales done on the month of February to be considered.
...
More complex queries are also possible, namely, our specification language allows specifying efficient views for most of TPC-H's queries. 
\carla{I agree with the comment, be more precise.}
%TODO: Maybe be more precise? Out of 23 queries, I have confirmed we can support 20 with our language - only Q13, Q16 and Q21 are not supported. And those 3 can still be supported in PotionDB by defining the views manually
...
\emph{\textbf{Extensions.}} 
We extend the specification language as follows.
On the \qcr{FROM}  clause, multiple containers are supported, as long as each object in the first container of the clause matches with, at most, one object from each of the other containers.
