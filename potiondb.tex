...
\begin{compactitem}
	\item begin(clk) $\rightarrow$ txId: starts a transaction and returns an identifier ($txId$) representing it. $TxId$ is used to associate reads and updates to this transaction, while $clk$ ensures applications see a casually evolving state of the database. We detail this further in Section \ref{sec:transactions};
	\item get(txId, id) $\rightarrow$ value: returns the full state of object $o^{id}$ in the context of transaction $txId$. E.g., if $o^{id}$ is a set, then all of its elements are returned;
	\item read(txId, id, op) $\rightarrow$ value: executes a read operation $op$ on object $o^{id}$ in the context of transaction $txId$, returning the result of $op$. E.g., if $o^{id}$ is a set, $lookup(a)$ checks if element $a$ is in the set;
	\item update(txId, id, op) $\rightarrow$ ok: associates an update of object $o^{id}$ to transaction $txId$. $Op$ represents the operation to be applied to modify $o^{id}$'s state. Updates are executed upon commit. If $o^{id}$ does not exist, it is created on commit;
	\item commit(txId) $\rightarrow$ clk: commits transaction $txId$, applying all associated updates.
	The clock generated by commit is returned for use in later transactions;
	\item abort(txId) $\rightarrow$ ok: cancels transaction $txId$, discarding all associated updates;
	\item readTx(clk, read(s)) $\rightarrow$ clk, values...: executes a read-only transaction with one or more reads;
	\item updateTx(clk, update(s)) $\rightarrow$ clk: executes a write-only transaction with one or more updates.
\end{compactitem}
...
The internal architecture of PotionDB is inspired by Cure and is split into three main components:
...
\begin{compactitem}
	...
	It coordinates transactions, implementing a transactional protocol (described in Section \ref{sec:transactions}) which ensures the consistency of reads and updates, as well as transactions' atomicity.
...
Many kinds of CRDTs have been proposed in the literature, forming a rich library of object types, e.g., counter, register, flag, set, map and topK (leaderboard). 
...
This avoids the need for ad-hoc mechanisms that are error-prone to compensate anomalies \cite{crdt, cure}.
...
E.g., in a map, both single and group additions/removals can be executed; for reads, the whole map can be read, directly check if a certain key exists, get the value of a key, etc.
...
These extensions and multi-operations, as well as the variety of data types, makes CRDT's implementation in PotionDB cumbersome, as it is necessary to ensure the states converge despite the execution of operations concurrently, as well as respect the sequential semantics of the object.
...
Generated views are read in the same way as any other object.
...
\begin{figure}
	...
	\begin{minipage}{0.19\textwidth}
		...
		\end{tabular}
		\vspace{1em}
		\captionof{table}{TopSales materialized view for February. This view builds upon Sales objects.}
...
The attributes for the view can potentially be one of the following aggregating functions of objects' attributes: \emph{sum}, \emph{count}, \emph{max}, \emph{min}, \emph{avg}.
\emph{Where} allows to define conditions that are used to decide whenever a given object is considered for the view or not, depending on certain attributes of the object (e.g., only include European customers in the view).
\emph{Order by} sorts the rows of the view given one or more attributes of the view.
...
For conditions in \emph{where}, $>$, $<$, $=$, $\neq$ are supported, as well as some operations with strings (e.g., startsWith). %Should we mention in theory we could support directly comparing dates?
A basic "if then else" is also supported inside aggregates, to further boost their expressivity.	%This is useful e.g. for Q14, that way we can use one single average object instead of two.
...
The \emph{From} clause then refers to one (and only one) container. %\footnote{PotionDB actually supports more than one container in \emph{From}, as long as each object from the first container matches exactly one object of each of the remaining containers. We discuss this further in Section \ref{subsec:limitations}.}. 
...
Figure \ref{fig:topSalesSQL} shows the specification of a view which answers the following query: "Who were the top 10 most profitable customers on any given month of 2023? And, for each of the top clients of said month, how many products did each client buy?"
...
The \emph{From} clause specifies that all objects belonging to the container \emph{sales} should be considered.
\emph{Select} lists which attributes should be part of the view.
\emph{Where} ensures only sales done on 2023 are considered, with the parameter [MONTH] ensuring the results are separated per month.
...
On the \emph{From} clause, multiple containers are supported, as long as each object from the first container matches, through condition(s) defined in \emph{Where}, exactly one object of each of the other containers.
...
If both operations are a write for the same object, this could lead to replicas' state diverging forever \cite{cops, cure}, i.e., returning different results for the same read forever.
...
However, on write-heavy scenarios (e.g., 75\%-25\% write-read), the provision of consistent snapshots for TCC is rather expensive.
...
Intuitively, under RC, PotionDB returns for each object whichever (commited) version is the latest available.
...
Many existing solutions shard the data across multiple machines in the same data center \cite{cops, eiger, cure, mdcc, gentlerain}.
...
Let $S_1$, ..., $S_n$ be the set of servers in the system.
Each server $S_j$ keeps a global vector clock $vc_G$, with one entry for each $S_k \in S_1, ..., S_n$.
This clock represents the latest snapshot available in $S_j$.
Each shard $sh_i$ also keeps a local vector clock $vc_i$.
The local vector clock simbolizes the latest snapshot available in $sh_i$, which may be different from $vc_G$.
Any shard can access the server's physical clock, $pc$.
Each shard $sh_i$ also keeps a list of prepared vector clocks, $prep_i$, as well as a list of commits on hold, $hold_i$.
...
In fact, any monotonically increasing clock would suffice for implementing $pc$.
...
Each $sh_i$ replies with the value of $pc$ at the time the message is processed and also stores it in $prep_i$. %Do I need to say it also stores the transaction ID?
...
%	\end{enumerate*}
	...
	\item Clients communicate with a server using Google’s protobufs \cite{???} to serialize/deserialize data. PotionDB’s protobuf interface is an extension of AntidoteDB’s protobuf interface \cite{???}.
...
\begin{enumerate}
	...
	\item\label{enum:question4}How does each kind of CRDT scale in PotionDB as we vary the number of clients, the number of replicas and other CRDT-specific settings (e.g., number of top elements in Top-K CRDT)?
...
Each client picks a CRDT randomly from an uniform distribution and uses said CRDT to do query and/or update operations. 
...
On the previous scenarios, each client only requested one query at a time, even through updates were executed in groups (whenever one update to a base object is usued, all associated views are updated in the same transaction).
...
However, the CAP theorem \cite{cap} implies that availability may be compromised in case of network partitions. Furthermore, high latency is to be expected in geo-distributed scenarios as multiple data centers need to be contacted in order to commit a transaction.
...
To the best of our knowledge, PotionDB is the first system to support materialized views in a patially geo-replicated scenario.
...
Partial replication allows to reduce storage costs and can even improve a system's performance (e.g., due to less operations being applied on each server) \cite{sipre, optimisticPartial, coda}, and are thus of interest for geo distribution.
The partitioning of the data must be carefully done by considering where each object is relevant, as otherwise clients may need to access far away servers and thus experience high latency \cite{sipre, optimisticPartial, coda}
However, one common problem with partial replication is that, sometimes, it is necessary to access data sharded among different data centers \cite{sipre}.
