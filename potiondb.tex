...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\State {receivingTxs}: sID $\to$ tID$\times$VC$\times$VC$\times$(sID $\to$ \textsf{list}$\langle$OP$\rangle$) %For each (remote) server, keeps the data of the transaction being received. This is needed as each transaction is sent split by buckets.
		\LeftComment{executes periodically}
		\Function{replicate}{ }
		\ForAll{sh \textbf{in} sID}
		\State \textbf{send} \Call{GetTransactions}{{lastSentClk}} \textbf{to} {sh}	
		...
		\State $\mathit{txCache}$.\Call{sortedAdd}{shardId,$\mathit{txns}$} adds to the list of pairs sorted by t.ct and shardID
		\EndFor{}
		...
		\Function{receiveBucket}{$\mathit{\textrm{Bucket } bucket, \textrm{[Update] } upds}$, $\mathit{\textrm{Clock } rc, \textrm{Timestamp } ct, \textrm{Id } txId, \textrm{Id } serverId}$}
		...
		\EndIf
		\State $\mathit{receivingTxns[serverId].txId, .rc, .ct = txId, rc, ct}$
...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\Function{remoteTxn}{$\mathit{\textrm{Id }txId, \textrm{Clock }rc, \textrm{Timestamp }ct}$, Id$\to$[Update] $\mathit{updsPerShard}$, Id $\mathit{serverId}$}
		\If{$\mathit{rc \leq vc_G}$}	\label{alg_line:remoteCheck}
		\For{\textbf{each} $\mathit{sh, upds} \in \mathit{updsPerShard}$}
		\State send \Call{applyRemoteTx}{$\mathit{ct, upds, serverId}$ to $\mathit{sh}$}
		\EndFor
		\ForAll{$\mathit{sh} \in \mathit{updsPerShard}$}
		\State wait for sh to finish or sh confirmation or sh to reply
		\EndFor
		\State $\mathit{vc_G[serverId]} = \mathit{ct}$
		\State \Call{checkPendingRemote}{}()
		\Else
		\State $\mathit{queued[serverId]}$.\Call{append}{$\mathit{ct, txId, rr, updsPerShard}$}
		\EndIf
		\EndFunction
		...
		\State send \Call{applyRemoteTx}{$\mathit{txs[0].ct, upds}$, $\mathit{serverId}$} to $\mathit{sh}$
		\EndFor
		\ForAll{$\mathit{sh} \in \mathit{txs[0].updsPerShard}$}
		\State wait to finish
		\EndFor
		\State $\mathit{vc_G[serverId] = txs[0].ct}$
		\State $\mathit{finished = false}$
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{applyRemoteTx}{$\mathit{\textrm{Timestamp }ct, \textrm{[Update] }upds}$, Id $\mathit{serverId}$}
		\Statex \hspace*{1.5em} (...)	\Comment{Applying the updates to the CRDTs}
		\State $\mathit{vc_j[serverId] = ct}$
		\EndFunction
		\Function{applyRemoteClk}{$\mathit{\textrm{Timestamp }ct, \textrm{Id }serverId}$}
		\State $\mathit{vc_j[serverId] = ct}$
		\EndFunction
