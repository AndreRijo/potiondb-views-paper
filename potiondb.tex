...
Note that despite PotionDB supporting views to be partially replicated (as evidenced by \textit{Local}), we replicate \textit{local} views everywhere in \textit{PotionDB}. %Andre: added this last phrase as for some reviewers it is not clear if local views are replicated everywhere. In the TPC-H  benchmarks, in PotionDB (not local/single), even local views are replicated everywhere. This was just because of convenience in implementing the client... so I do not know how to justify it here. Should I just adapt the experiments for Global to only replicate the local views locally?
...
\begin{figure}
	\centering
	\includegraphics[width=0.5\linewidth]{postgres/postgresNoView}
	%\vspace*{-0.65em}
	\caption{PostgreSQL query performance with indexes instead of materialized views. Notice that the x axis is not in the scale of thousands.}
