...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\Statex \quad hold$_i$: \code{heap}$\langle$tID$\times$TC$\times$TC$\rangle$ \Comment{commits on hold (txId, rc, cc)} 
		...
		\Statex \quad txUpds: tID $\to$ \code{list}$\langle$Upds$\rangle$ \Comment{Transaction updates}
