...
\begin{algorithm}
	\begin{algorithmic}[1]
		\State $\mathit{vcG}$ \Comment{global vector clock of $S_i$}
		\State $\mathit{txInfo}$ \Comment{$\mathit{txID} \to \{\mathit{shID}\}$}
		\Function{startTransaction}{Clock $\mathit{clientClk}$}
			\State wait until $\mathit{clientClk} \leq \mathit{vcG}$
			\State $\mathit{rc} = \mathit{vcG}$
			\State $\mathit{txId} = $ new unique id
			\State $\mathit{txInfo[txId]} = \{\}$
			\State \textbf{return} $\mathit{txId, rc}$
		\EndFunction
		\Function{updateOp}{Id $\mathit{txId}$, Update $\mathit{upd}$}
			\State $\mathit{txnInfo[txId]} = \mathit{txnInfo[txId]} \cup \mathit{upd^{sh}}$	%Maybe on Sharding subsection I should mention we can get the shard of any object, update or read by doing ^{shard} (instead of just saying of any object)
			\State send \Call{update}{$\mathit{txId}, \mathit{upd}$} to $\mathit{upd^{sh}}$
		\EndFunction
		\Function{readOp}{Id $\mathit{txId}$, Clock $\mathit{rc}$, Read $\mathit{read}$}
			\State send \Call{read}{$\mathit{txId}, \mathit{rc}, \mathit{read}$} to $\mathit{read^{sh}}$
		\EndFunction
		\Function{commitTx}{Id $\mathit{txId}$, Clock $\mathit{rc}$}
			\For{\textbf{each} $\mathit{sh} \in \mathit{txInfo[txId]}$}	\Comment{W\textsubscript{T}}
				\State send \Call{prepare}{$\mathit{txId}$} to $\mathit{sh}$
			\EndFor
			\State $\mathit{maxTs} = \bot$	\Comment{timestamp, not vector clock}
			\For{\textbf{each} $\mathit{sh} \in \mathit{txInfo[txId]}$}	
				\State $\mathit{suggestedTs} \gets$ reply of $\mathit{sh}$ to \textsc{prepare}
				\State $\mathit{maxTs} =$ \Call{max}{$\mathit{maxTs, suggestedTs}$}
			\EndFor
			\For{\textbf{each} $\mathit{sh} \in \mathit{txInfo[txId]}$}	
				\State send \Call{commit}{$\mathit{txId, rc, maxTs}$} to $\mathit{sh}$
			\EndFor
		\EndFunction
		\LeftComment{An always running routine}
		\Function{globalClockUpdate}{}()
			\ForAll{$T \in $ ongoing transactions}
				\State wait \textbf{for all} $\mathit{sh} \in \mathit{txInfo[T.txId]}$ to reply
				\State $\mathit{vcG[i]} =$ \Call{max}{$\mathit{vcG[i]}, T.cc$}
				...
				\State $\mathit{txInfo[T.txId]} = \bot$ 
			\EndFor
		\EndFunction
	\end{algorithmic}
	\caption{Commit and read protocol on S\textsubscript{i}'s Transaction Manager}
	\label{alg:commit_tm}
\end{algorithm}

\begin{algorithm}
	\begin{algorithmic}[1]
		\State $\mathit{vc_j}$ \Comment{latest applied clock of $\mathit{sh_j}$}
		\State $\mathit{prep_i}$ \Comment{list of (sorted) prepared timestamps}
		\State $\mathit{hold_i}$ \Comment{list of (sorted) commits on hold} %commits that have been confirmed but not yet applied (due to some timestamp in prepi that is smaller or another timestamp in holdi with a smaller value)
		\State $\mathit{txUpds}$ \Comment{$\mathit{txId} \to \mathit{[updates]}$}
		...
		\Function{prepare}{Id $\mathit{txId}$}
			\State $\mathit{ts} = \mathit{pc}$
			\State $\mathit{prep_i}$.\Call{sortedAdd}{$\mathit{ts, txId}$}
			\State reply to $\mathit{TM}$ with $\mathit{ts}$
		\EndFunction
		\Function{commit}{$\mathit{\textrm{Id } txId, \textrm{Clock } rc, \textrm{Timestamp } cc}$}
			\If{\Call{canCommit}{$\mathit{txId, cc}$}}
				\State \Call{applyCommit}{$\mathit{txId, rc, cc}$}
			\Else
				\State $\mathit{hold_i}$.\Call{sortedAdd}{$\mathit{cc, txId, rc}$}
			\EndIf
			\State \Call{checkHoldCommits}{}()	%Even if txID’s was put on hold, it may have been the lowest prepare and, thus, another commit on hold may be OK to be applied now 
		\EndFunction
		\Function{canCommit}{$\mathit{\textrm{Id } txId, \textrm{Timestamp } maxTs}$}
			\State $\mathit{prep_i}$.\Call{remove}{$\mathit{txId}$}
			\If{$\mathit{prep_i}$.\Call{isEmpty}{}() \&\& $\mathit{hold_i}$.\Call{min}{}().$\mathit{cc} < \mathit{maxTs}$}
				\State \textbf{return} false
			\EndIf
			\If{$\mathit{prep_i}$.\Call{min}{}().$\mathit{ts < maxTs}$ $\|$ ($\mathit{hold_i}$.\Call{notEmpty}{}() \&\& $\mathit{hold_i}$.\Call{min}{}().$\mathit{cc < maxTs}$)}
				\State \textbf{return} false
			\EndIf
			\LeftComment{Safe to commit as no $T$ in $\mathit{prepi}$ or $\mathit{holdi}$ is smaller than $\mathit{maxTs}$}
			\State \textbf{return} true
		\EndFunction
		\LeftComment{I don't think we need the pseudo-code for this function.}
		\Function{checkHoldCommits}{}()
			...
			\If{$\mathit{hold_i}$.\Call{isEmpty}{}()}
				\State \textbf{return}
			\EndIf
			\If{$\mathit{prep_i}$.\Call{isEmpty}{}()}
				\While{$\mathit{hold_i}$.\Call{notEmpty}{}()}
					\State $\mathit{txId, rc, cc = hold_i}$.\Call{removeMin}{}()
					\State \Call{applyCommit}{$\mathit{txId, rc, cc}$}
				\EndWhile
				\State \textbf{return}
			\EndIf
			\State $\mathit{minPrep = prep_i}$.\Call{min}{}().$\mathit{ts}$
			\For{\textbf{each} ($\mathit{txId, rc, cc}$) in $\mathit{hold_i}$}
				\If{$\mathit{cc < minPrep}$}
					\State $\mathit{holdi}$.\Call{removeMin}{}()	\Comment{Same as the current value of $\mathit{txId, rc, cc}$}
					\State \Call{applyCommit}{$\mathit{txId, rc, cc}$}
				\Else
					\State \textbf{return}
				\EndIf
			\EndFor
		\EndFunction
		\Function{applyCommit}{$\mathit{\textrm{Id } txId, \textrm{Clock } rc, \textrm{Timestamp } maxTs}$}
			...
			\State $\mathit{vc_j[i] = maxTs}$
			\State $\mathit{txUpds[txId] = \bot}$
			\State reply to $\mathit{TM}$ (for \Call{globalClockRoutine}{})
		\EndFunction
		\Function{update}{$\mathit{\textrm{Id } txId, \textrm{Update } upd}$}
			\State $\mathit{txUpds[txId]}$.\Call{append}{$\mathit{upd}$}
		\EndFunction
		\Function{applyRead}{$\mathit{\textrm{Id } txId, \textrm{Clock } rc, \textrm{Read } read}$}
			\If{$\mathit{rc \leq vc_j}$ \&\& $\mathit{rc \geq objs[read.id]}$.\Call{lastVersion}{}()}
				\State \textbf{return} 	$\mathit{objs[read.id]}$.\Call{read}{$\mathit{txUpds[txId]}$}
			\EndIf
			\State \textbf{return} $\mathit{objs[read.id]}$.\Call{readInThePast}{$\mathit{txUpds[txId]}$}
		\EndFunction
	\end{algorithmic}
	\caption{Commit and read protocol execution on S\textsubscript{i}'s shard sh\textsubscript{j}}
	\label{alg:commit_part}
...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\Function{replicate}{}()
			...
			\EndFor
			\State $\mathit{txCache = [(shardId \to T)]}$
			...
			\ForAll{shards $\mathit{sh}$}
				...
				\State $\mathit{txCache}$.\Call{sortedAdd}{$\mathit{txns}$}
			\EndFor{}
			\For{\textbf{each} $\mathit{txShards} \in \mathit{txCache}$ \&\& $\mathit{txShards.cc} \leq \mathit{safeClk}$}
				\State $\mathit{txPerBucket =}$ \Call{splitTxPerBucket}{$\mathit{txShards}$}
				\For{\textbf{each} $\mathit{bucket, upds} \in \mathit{txPerBucket}$}
					\State send ($\mathit{bucket, upds, txShards.rc, txShards.cc}$,   $\mathit{txShards.txId}$) to servers replicating $\mathit{bucket}$	%In our prototype, send this to the RabbitMQ instance of Si.
				\EndFor
			\EndFor
...
\begin{algorithm}
	\begin{algorithmic}[1]
		\State $\mathit{queued}$ \Comment{$serverId \to [T | CLK]$}	%Since for each server the transactions arrive by T.cc order, they are also stored by T.cc order.
		...
		\Function{remoteTxn}{$\mathit{\textrm{Id }txId, \textrm{Clock }rr, \textrm{Timestamp }cc}$, Id$\to$[Update] $\mathit{updsPerShard}$, Id $\mathit{serverId}$}
			\If{$\mathit{rr \leq vc_G}$}
	...
	\end{algorithmic}
	\caption{Remote transactions on S\textsubscript{i}'s Transaction Manager}
