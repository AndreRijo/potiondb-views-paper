...
\begin{itemize}
	...
	This can lead to great pressure in the server with the view when updating objects related to the view and when querying views, as well as worse latency to faraway clients.	
\end{itemize}
...
\andre{I'll need to explain the redirection somewhere. I think there was already a section with some details of this, but I'll need to revisit it.}
...
\andre{TODO: Update this paragraph after the new update vs query graphs are added.}
...
\begin{enumerate}
	...
	\item\label{enum:question4}\andre{Should I do some single CRDT/server benchmarks (i.e., not TPC-H)?}
...
\andre{I need a reference here to the redirection system in the article}.
Due to the redirection of requests (see Section \ref{???}) that occours due to not all data required being in the server contacted by a client, this implies an extra round-trip for some requests, which further slows down performance.
...
\begin{figure}
	...
	\caption{PotionDB performance executing all queries with latency. Both axes have a log(10) scale. \andre{Still doing more latency tests, will update the ``local line'' with more points later on.}}
...
This would not affect query throughput, just the freshness of data. \andre{I'll need to include a test for this in the next section.}
...
Compared to Figure \ref{fig:global_local_single}, the max throughput is much lower. This is due to the much higher amount of clients needed to sature the servers: (\andre{Note: this value needs to be updated}) aproximately 3000 clients instead of 160.
...
\andre{Text from here on is old text}
