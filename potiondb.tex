...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\Statex \textbf{input}
		\Statex $\quad$ globalVC: vClock  \Comment{global vector clock of $S_i$}
		...
		\Function{commitTx}{txId: ID, readVC: vClock}
			\ForAll{sh \textbf{in} txInfo[txId]	} \Comment{all shards written by txId}
				\State \textbf{send} \Call{prepare}{txId} \textbf{to} sh
			\EndFor
			...
			\ForAll{sh \textbf{in} txInfo[txId]}	
				...
				\State maxTs $\leftarrow$ \Call{max}{maxTs, suggestedTs}
			\EndFor
			\ForAll{sh \textbf{in} txInfo[txId]}	
				\State \textbf{send} \Call{commit}{txId, readVc, maxTs} \textbf{to} sh
			\EndFor
		\EndFunction
		...
		\Function{globalClockUpdate}{ }
			\ForAll{T \textbf{in} ongoingTxs}
				...
				\State vcG[i \begin{color}{red}(what is i?)\end{color}]  $\leftarrow$ \Call{max}{vcG[i], T.cc}
		...
		\EndFunction
	\end{algorithmic}
	\caption{S\textsubscript{i}'s Transaction Manager: commit and read protocol}
	\label{alg:commit_tm}
\end{algorithm}
