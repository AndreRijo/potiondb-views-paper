...
%	\begin{algorithmic}[1]
%		\State oneday$^{TM}$
%		\Function{update}{Identifier $\mathit{txId}$, Updates[] $\mathit{upds}$}
%			\For{$\mathit{upd}$ \textbf{in} $\mathit{upds}$}
%				 \State $\mathit{txnInfo[txID]}$ = $\mathit{txnInfo[txID]} \cup \mathit{upd^{shard}}$	%Maybe on Sharding subsection I should mention we can get the shard of any object, update or read by doing ^{shard} (instead of just saying of any object)
%			\EndFor
%		\EndFunction
%	\end{algorithmic}
%	\caption{WIP}
