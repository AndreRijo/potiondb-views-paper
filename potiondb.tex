...
In these experiments, we used PostgreSQL incremental view maintenance extension \cite{ivm}. 
As this extension cannot be used with PostgreSQL's Logical Replication \cite{PostgreSQLLogical}, our configuration with PostgreSQL can only
use a single server, with all operation being propagated to this server. Thus, we focus on comparing PostgreSQL with \emph{Single}.
