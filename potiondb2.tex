...
but its primary goal is to support incremental view maintenance.%, as detailed in Section~\ref{sec:views_for_apps}.
%In the next section
...
maintenance, providing TCC access to the base and view objects.
...
Our evaluation shows that PotionDB can answer complex queries with lower latency and highher throughput when 
compared with alternative approaches, even with reasonable update ratios.


%PotionDB provides transactional causal consistency for transactions accessing 
%both base objects and materialized view objects.
%Materialized views can be used to efficiently answer recurrent complex queries over data that is partitioned
